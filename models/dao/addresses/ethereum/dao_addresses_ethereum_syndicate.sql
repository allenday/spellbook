...
syndicatev1_daos as ( -- getting investment clubs created on dune v1 
        SELECT 
            block_time, 
            CONCAT('0x', RIGHT(topic1, 40)) as dao 
        FROM 
...
syndicate_wallets as (
        SELECT 
            TIMESTAMP_TRUNC(ad.block_time, day) as created_date, 
        ...
        latest_wallet lw 
            ON ad.dao = lw.dao
        WHERE lw.change_rank IS NULL AND lw.change_rank = 1 -- getting the most recent owner 
)
