...
SELECT
    ...
    CASE
        WHEN get_json_object(bm.buy, '$.amount') = 1 THEN 'Single Item Trade'
        ELSE 'Bundle Trade'
    END AS trade_type,
    ...
    CASE
        WHEN get_json_object(bm.buy, '$.matchingPolicy') IN ('0x00000000006411739da1c40b106f8511de5d1fac', '0x0000000000dab4a563819e8fd93dba3b25bc3495') THEN 'Buy'
    ...
    CASE
        WHEN get_json_object(bm.buy, '$.paymentToken') IN ('0x0000000000000000000000000000000000000000', '0x0000000000a39bb272e79075ade125fd351887ac') THEN CAST(get_json_object(bm.buy, '$.price') / POWER(10, 18) AS double)
        ELSE CAST(get_json_object(bm.buy, '$.price') / POWER(10, pu.decimals) AS double)
    END AS amount_original,
    CASE
        WHEN get_json_object(bm.buy, '$.paymentToken') IN ('0x0000000000000000000000000000000000000000', '0x0000000000a39bb272e79075ade125fd351887ac') THEN CAST(pu.price * get_json_object(bm.buy, '$.price') / POWER(10, 18) AS double)
        ELSE CAST(pu.price * get_json_object(bm.buy, '$.price') / POWER(10, pu.decimals) AS double)
    END AS amount_usd,
    CASE
        WHEN get_json_object(bm.buy, '$.paymentToken') IN ('0x0000000000000000000000000000000000000000', '0x0000000000a39bb272e79075ade125fd351887ac') THEN 'ETH'
        ELSE pu.symbol
    END AS currency_symbol,
    CASE
        WHEN get_json_object(bm.buy, '$.paymentToken') IN ('0x0000000000000000000000000000000000000000', '0x0000000000a39bb272e79075ade125fd351887ac') THEN '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
    ...
    CASE
        WHEN get_json_object(bm.buy, '$.paymentToken') IN ('0x0000000000000000000000000000000000000000', '0x0000000000a39bb272e79075ade125fd351887ac') THEN CAST(COALESCE(get_json_object(bm.buy, '$.price') / POWER(10, 18) * get_json_object(get_json_object(bm.sell, '$.fees[0]'), '$.rate') / 10000, 0) AS double)
    ...
    CASE
        WHEN get_json_object(bm.buy, '$.paymentToken') IN ('0x0000000000000000000000000000000000000000', '0x0000000000a39bb272e79075ade125fd351887ac') THEN CAST(COALESCE(pu.price * get_json_object(bm.buy, '$.price') / POWER(10, 18) * get_json_object(get_json_object(bm.sell, '$.fees[0]'), '$.rate') / 10000, 0) AS double)
    ...
    CASE
        WHEN get_json_object(get_json_object(bm.sell, '$.fees[0]'), '$.recipient') IS NOT NULL AND get_json_object(bm.buy, '$.paymentToken') IN ('0x0000000000000000000000000000000000000000', '0x0000000000a39bb272e79075ade125fd351887ac') THEN 'ETH'
...
FROM {{ source('blur_ethereum','BlurExchange_evt_OrdersMatched') }} AS bm
INNER JOIN {{ source('ethereum','transactions') }} AS et
    ON
        et.block_number = bm.evt_block_number
    ...
    {% endif %}
        ...
        {% endif %}
LEFT JOIN {{ ref('nft_ethereum_aggregators') }} AS agg ON agg.contract_address = et.to
LEFT JOIN {{ ref('nft_ethereum_aggregators_markers') }} AS agg_m
    ON RIGHT(et.data, agg_m.hash_marker_size) = agg_m.hash_marker
LEFT JOIN {{ source('prices','usd') }} AS pu
    ON
        pu.blockchain = 'ethereum'
        AND pu.minute = date_trunc('minute', bm.evt_block_time)
        AND (
            pu.contract_address = get_json_object(bm.buy, '$.paymentToken')
    ...
    {% endif %}
        ...
        {% endif %}
LEFT JOIN {{ ref('tokens_ethereum_nft') }} AS nft ON get_json_object(bm.buy, '$.collection') = nft.contract_address
{% if is_incremental() %}
...
SELECT
    ...
    CASE
        WHEN get_json_object(s.consideration[0], '$.recipient') != s.recipient THEN get_json_object(s.consideration[0], '$.recipient')
...
FROM {{ source('seaport_ethereum','Seaport_evt_OrderFulfilled') }} AS s
INNER JOIN {{ source('ethereum', 'transactions') }} AS tx
    ON
        tx.block_number = s.evt_block_number
...
{% if not is_incremental() %}
    ...
    {% endif %}
LEFT JOIN {{ ref('tokens_ethereum_nft') }} AS nft_tok ON nft_tok.contract_address = get_json_object(s.offer[0], '$.token')
LEFT JOIN {{ ref('tokens_ethereum_erc20') }} AS currency_tok ON currency_tok.contract_address = get_json_object(s.consideration[0], '$.token')
LEFT JOIN {{ ref('prices_usd_forward_fill') }} AS pu
    ON (
        (pu.contract_address = get_json_object(s.consideration[0], '$.token') AND pu.blockchain = 'ethereum')
...
{% if not is_incremental() %}
    ...
    {% endif %}
WHERE
    s.zone = '0x0000000000d80cfcb8dfcd8b2c4fd9c813482938'
    {% if is_incremental() %}
