...
WITH all_foundation_trades AS (
    ...
    , case when (f.sellerRev = 0 and cast(f.creatorRev as BIGNUMERIC) > 0 ) then 'primary' else 'secondary' end as trade_type
    , f.seller
    , f.bidder AS buyer
    ...
    , case when (f.sellerRev = 0 and cast(f.creatorRev as BIGNUMERIC) > 0 ) then 0 else f.creatorRev end AS royalty_fee_amount_raw
    ...
    INNER JOIN {{ source('foundation_ethereum','market_evt_ReserveAuctionCreated') }} c ON c.auctionId=f.auctionId AND c.evt_block_time<=f.evt_block_time
     ...
     WHERE f.evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
     {% else %}
    ...
    , case when (sellerRev = 0 and CAST(creatorRev AS BIGNUMERIC) > 0 ) then 'primary' else 'secondary' end as trade_type
    , seller
    , buyer
    ...
    , case when (sellerRev = 0 and CAST(creatorRev AS BIGNUMERIC) > 0 ) then 0 else creatorRev end AS royalty_fee_amount_raw
    ...
    , case when (sellerRev = 0 and CAST(creatorFee AS BIGNUMERIC) > 0 ) then 'primary' else 'secondary' end as trade_type
    , seller
    , buyer
    ...
    , case when (sellerRev = 0 and CAST(creatorFee AS BIGNUMERIC) > 0 ) then 0 else creatorFee end AS royalty_fee_amount_raw
    , evt_index as sub_tx_trade_id
...
, t.seller
, t.buyer
, cast(t.price_raw as BIGNUMERIC) as price_raw
, '{{ var("ETH_ERC20_ADDRESS") }}' AS currency_contract -- all trades are in ETH
, t.project_contract_address
, t.nft_contract_address
, t.tx_hash
, cast(t.platform_fee_amount_raw as BIGNUMERIC) as platform_fee_amount_raw
, cast(t.royalty_fee_amount_raw as BIGNUMERIC) as royalty_fee_amount_raw
