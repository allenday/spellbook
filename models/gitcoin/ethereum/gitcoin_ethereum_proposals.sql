...
FROM
    {{ source('gitcoin_ethereum', 'GovernorAlpha_evt_ProposalCreated') }} AS pcr
LEFT JOIN cte_sum_votes ON cte_sum_votes.proposalId = pcr.id
LEFT JOIN
    {{ source('gitcoin_ethereum', 'GovernorAlpha_evt_ProposalCanceled') }} AS pca ON
        pca.id = pcr.id
LEFT JOIN
    {{ source('gitcoin_ethereum', 'GovernorAlpha_evt_ProposalExecuted') }} AS pex ON
        pex.id = pcr.id
LEFT JOIN
    {{ source('gitcoin_ethereum', 'GovernorAlpha_evt_ProposalQueued') }} AS pqu ON
        pex.id = pcr.id
{% if is_incremental() %}
