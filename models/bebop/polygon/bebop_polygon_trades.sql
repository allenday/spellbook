...
WITH
  bebop_raw_data AS (
    ...
    FROM
      {{ source('bebop_v3_polygon', 'BebopAggregationContract_evt_AggregateOrderExecuted') }}
      LEFT JOIN {{ source('bebop_v3_polygon', 'BebopAggregationContract_call_SettleAggregateOrder') }} ex ON ex.call_tx_hash = evt_tx_hash
    WHERE
      call_success = true
