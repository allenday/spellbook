...
WITH all_foundation_trades AS (
    SELECT
        ...
        f.creatorrev / POWER(10, 18) AS royalty_fee_amount,
        f.evt_index
    FROM {{ source('foundation_ethereum','market_evt_ReserveAuctionFinalized') }} AS f
    LEFT JOIN {{ source('foundation_ethereum','market_evt_ReserveAuctionCreated') }} AS c ON c.auctionid = f.auctionid AND c.evt_block_time <= f.evt_block_time
...
SELECT
    ...
    CASE
        WHEN nft_t.contract_address IS NOT NULL THEN 'erc721'
        ELSE 'erc1155'
    END AS token_standard,
    CASE
        WHEN agg.contract_address IS NOT NULL THEN 'Bundle Trade'
    ...
    CASE
        WHEN t.royalty_fee_amount / t.amount_original < 0.5 THEN CAST(t.royalty_fee_amount_raw AS DOUBLE)
        ELSE CAST(0 AS DOUBLE)
    END AS royalty_fee_amount_raw,
    CASE
        WHEN t.royalty_fee_amount / t.amount_original < 0.5 THEN t.royalty_fee_amount
        ELSE 0
    END AS royalty_fee_amount,
    CASE
        WHEN t.royalty_fee_amount / t.amount_original < 0.5 THEN t.royalty_fee_amount * pu.price
        ELSE 0
    END AS royalty_fee_amount_usd,
    CASE
        WHEN t.royalty_fee_amount / t.amount_original < 0.5 THEN CAST(100.0 * ROUND(t.royalty_fee_amount / t.amount_original, 2) AS DOUBLE)
        ELSE CAST(0 AS DOUBLE)
...
LEFT JOIN {{ ref('tokens_ethereum_nft') }} AS nft ON t.nft_contract_address = nft.contract_address
LEFT JOIN {{ source('foundation_ethereum','market_evt_SellerReferralPaid') }} AS sellref ON sellref.evt_block_time = t.block_time AND sellref.evt_tx_hash = t.tx_hash
LEFT JOIN {{ source('erc721_ethereum','evt_transfer') }} AS nft_t
    ON
        nft_t.evt_block_time = t.block_time
...
LEFT JOIN {{ source('prices', 'usd') }} AS pu
    ON
        pu.minute = date_trunc('minute', t.block_time)
        AND pu.blockchain = 'ethereum'
        AND pu.contract_address = t.currency_contract
        {% if is_incremental() %}
            AND pu.minute >= date_trunc('day', now() - INTERVAL '1 week')
        {% endif %}
LEFT JOIN {{ source('ethereum','traces') }} AS ett
    ON
        ett.block_time = t.block_time
