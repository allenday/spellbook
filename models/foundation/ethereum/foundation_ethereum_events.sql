...
WITH all_foundation_trades AS (
    ...
    , CAST(f.totalFees AS FLOAT64) AS platform_fee_amount_raw
...
, CAST(100.0*ROUND(t.platform_fee_amount/t.amount_original, 2) AS FLOAT64) AS platform_fee_percentage
, CASE WHEN t.royalty_fee_amount/t.amount_original < 0.5 THEN CAST(t.royalty_fee_amount_raw AS FLOAT64)
...
, CASE WHEN t.royalty_fee_amount/t.amount_original < 0.5 THEN CAST(100.0*ROUND(t.royalty_fee_amount/t.amount_original, 2) AS FLOAT64)
    ELSE CAST(0 AS FLOAT64)
...
LEFT JOIN {{ source('prices', 'usd') }} pu ON pu.minute=TIMESTAMP_TRUNC(t.block_time, minute)
