...
WITH apeswap_dex AS (
    SELECT
        ...
        '' AS trace_address,
        t.evt_index
    FROM {{ source('apeswap_polygon', 'ApePair_evt_Swap') }} AS t
    INNER JOIN {{ source('apeswap_polygon', 'ApeFactory_evt_PairCreated') }} AS p
...
LEFT JOIN {{ ref('tokens_erc20') }} AS erc20a
    ON
        erc20a.contract_address = apeswap_dex.token_bought_address
        AND erc20a.blockchain = 'polygon'
LEFT JOIN {{ ref('tokens_erc20') }} AS erc20b
    ON
        erc20b.contract_address = apeswap_dex.token_sold_address
        AND erc20b.blockchain = 'polygon'
LEFT JOIN {{ source('prices', 'usd') }} AS p_bought
    ON
        p_bought.minute = date_trunc('minute', apeswap_dex.block_time)
...
LEFT JOIN {{ source('prices', 'usd') }} AS p_sold
    ON
        p_sold.minute = date_trunc('minute', apeswap_dex.block_time)
