...
WITH deposit_events AS (
    SELECT
        ...
        CAST(bytea2numeric_v3(
            '0x' || SUBSTR(d.amount, 15, 2) || SUBSTR(d.amount, 13, 2) || SUBSTR(d.amount, 11, 2) || SUBSTR(d.amount, 9, 2)
            || SUBSTR(d.amount, 7, 2) || SUBSTR(d.amount, 5, 2) || SUBSTR(d.amount, 3, 2)
        ) / POWER(10, 9
        ) AS DOUBLE) AS amount,
        d.contract_address,
        CAST(bytea2numeric_v3(
            '0x' || SUBSTR(d.index, 15, 2) || SUBSTR(d.index, 13, 2) || SUBSTR(d.index, 11, 2) || SUBSTR(d.index, 9, 2)
            || SUBSTR(d.index, 7, 2) || SUBSTR(d.index, 5, 2) || SUBSTR(d.index, 3, 2)
        ) AS DECIMAL(38, 0
        )) AS deposit_index,
        d.pubkey,
        d.signature,
    ...
    FROM {{ source('eth2_ethereum', 'DepositContract_evt_DepositEvent') }} AS d
...
traces AS (
    ...
    FROM {{ source('ethereum', 'traces') }} AS t
    WHERE
...
INNER JOIN {{ source('ethereum', 'transactions') }} AS et
    ON
        et.block_number = d.block_number
