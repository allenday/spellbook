...
LEFT JOIN {{ ref('ironbank_optimism_itokens') }} i ON CAST(m.contract_address AS STRING) = i.contract_address
LEFT JOIN {{ source('prices', 'usd') }} p ON p.minute = TIMESTAMP_TRUNC(m.evt_block_time, minute) AND CAST(p.contract_address AS STRING) = i.underlying_token_address AND p.blockchain = 'optimism'