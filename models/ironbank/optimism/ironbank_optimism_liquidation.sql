...
LEFT JOIN (
    ...
    FROM {{ ref('ironbank_optimism_itokens') }}
) AS i_collateral
...
LEFT JOIN {{ ref('ironbank_optimism_itokens') }} AS i ON CAST(l.contract_address AS VARCHAR(100)) = i.contract_address
LEFT JOIN {{ source('prices', 'usd') }} AS p ON p.minute = date_trunc('minute', l.evt_block_time) AND CAST(p.contract_address AS VARCHAR(100)) = i.underlying_token_address AND p.blockchain = 'optimism'
