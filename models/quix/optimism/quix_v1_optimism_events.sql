...
with events_raw as (
    ...
    where
        erc721address != lower('0xbe81eabdbd437cba43e4c1c330c63022772c2520') -- --exploit contract
...
transfers as (
    ...
    inner join {{ ref('transfers_optimism_eth') }} as tr
        on
            er.tx_hash = tr.tx_hash
    ...
    select
        ...
        erc20.to
    from events_raw as er
    inner join {{ source('erc20_optimism','evt_transfer') }} as erc20
        on
            er.tx_hash = erc20.evt_tx_hash
...
final as (
    ...
    left join {{ source('erc20_optimism','evt_transfer') }} as erc20
        on
            erc20.evt_block_time = er.block_time
