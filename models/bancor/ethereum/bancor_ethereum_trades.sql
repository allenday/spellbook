...
WITH conversions AS (
    SELECT
        ...
        t.evt_tx_hash,
        t.evt_index
    FROM {{ source('bancornetwork_ethereum', 'BancorNetwork_v6_evt_Conversion') }} AS t
    {% if is_incremental() %}
    ...
    SELECT
        ...
        t.evt_tx_hash,
        t.evt_index
    FROM {{ source('bancornetwork_ethereum', 'BancorNetwork_v7_evt_Conversion') }} AS t
    {% if is_incremental() %}
        WHERE t.evt_block_time >= date_trunc("day", now() - interval "1 week")
    {% endif %}
    {% if not is_incremental() %}
    ...
    SELECT
        ...
        t.evt_tx_hash,
        t.evt_index
    FROM {{ source('bancornetwork_ethereum', 'BancorNetwork_v8_evt_Conversion') }} AS t
    {% if is_incremental() %}
        WHERE t.evt_block_time >= date_trunc("day", now() - interval "1 week")
    {% endif %}
    {% if not is_incremental() %}
    ...
    SELECT
        ...
        t.evt_tx_hash,
        t.evt_index
    FROM {{ source('bancornetwork_ethereum', 'BancorNetwork_v9_evt_Conversion') }} AS t
    {% if is_incremental() %}
        WHERE t.evt_block_time >= date_trunc("day", now() - interval "1 week")
    {% endif %}
    {% if not is_incremental() %}
    ...
    SELECT
        ...
        t.evt_tx_hash,
        t.evt_index
    FROM {{ source('bancornetwork_ethereum', 'BancorNetwork_v10_evt_Conversion') }} AS t
    {% if is_incremental() %}
        WHERE t.evt_block_time >= date_trunc("day", now() - interval "1 week")
    {% endif %}
    {% if not is_incremental() %}
...
dexs AS (
    ...
    SELECT
        ...
        "" AS trace_address,
        t.evt_index
    FROM {{ source('bancor3_ethereum', 'BancorNetwork_evt_TokensTraded') }} AS t
    {% if is_incremental() %}
        WHERE t.evt_block_time >= date_trunc("day", now() - interval "1 week")
    {% endif %}
    {% if not is_incremental() %}
