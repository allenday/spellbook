...
with manual_mapping as (
    select
        lp_name,
        LOWER(addr) as contract_address
    from (
        values
...
select distinct
    ...
    token0,
    token1
from manual_mapping as mm
inner join {{ source('optimism', 'creation_traces') }} as ct
...
inner join {{ source('optimism', 'logs') }} as l
    on
        t.hash = l.tx_hash
