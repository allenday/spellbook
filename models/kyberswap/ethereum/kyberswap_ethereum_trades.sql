...
kyberswap_dex AS (
    ...
    FROM {{ source('kyber_ethereum', 'DMMPool_evt_Swap') }} AS t
    INNER JOIN {{ source('kyber_ethereum', 'DMMFactory_evt_PoolCreated') }} AS p
    ...
    FROM {{ source('kyber_ethereum', 'Elastic_Pool_evt_swap') }} AS t
    INNER JOIN {{ source('kyber_ethereum', 'Elastic_Factory_evt_PoolCreated') }} AS p
        ON t.contract_address = p.pool
    {% if is_incremental() %}
        WHERE t.evt_block_time >= date_trunc('day', now() - interval '1 week')
    {% else %}
