...
kyberswap_dex AS (
    ...
    SELECT
        t.evt_block_time                                                               AS block_time
        ,t.sender                                                                      AS takerstartswith
        ,t.recipient                                                                   AS maker
        ,if(t.deltaQty0 < 0, t.deltaQty1, t.deltaQty0)                    AS token_bought_amount_raw
        ,ABS(if(t.deltaQty0 < 0, t.deltaQty0, t.deltaQty1))  AS token_sold_amount_raw
        ,cast(NULL as FLOAT64)                                                          AS amount_usd
        ,if(t.deltaQty0 < 0, p.token1, p.token0)                          AS token_bought_address
        ,if(t.deltaQty0 < 0, p.token0, p.token1)                          AS token_sold_address
        ,t.contract_address                                                            AS project_contract_address
        ,t.evt_tx_hash                                                                 AS tx_hash
        ,''                                                                            AS trace_address
        ,'elastic'                                                                     AS version	
...
SELECT
    ...
    ,version                                                               AS version
    ,SAFE_CAST(TIMESTAMP_TRUNC(kyberswap_dex.block_time, DAY) AS date)        AS block_date
    ...
    ,CAST(kyberswap_dex.token_bought_amount_raw AS BIGNUMERIC) AS token_bought_amount_raw
    ,CAST(kyberswap_dex.token_sold_amount_raw AS BIGNUMERIC) AS token_sold_amount_raw
...
LEFT JOIN {{ source('prices', 'usd') }} p_bought
    ON p_bought.minute = TIMESTAMP_TRUNC(kyberswap_dex.block_time, minute)
...
LEFT JOIN {{ source('prices', 'usd') }} p_sold
    ON p_sold.minute = TIMESTAMP_TRUNC(kyberswap_dex.block_time, minute)
