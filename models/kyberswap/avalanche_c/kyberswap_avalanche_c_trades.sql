...
kyberswap_dex AS (
    SELECT
        ...
        '' AS trace_address,
        t.evt_index
    FROM {{ source('kyber_avalanche_c', 'DMMPool_evt_Swap') }} AS t
    INNER JOIN {{ source('kyber_avalanche_c', 'DMMFactory_evt_PoolCreated') }} AS p
    ...
    SELECT
        ...
        '' AS trace_address,
        t.evt_index
    FROM {{ source('kyber_avalanche_c', 'Elastic_Pool_evt_swap') }} AS t
    INNER JOIN {{ source('kyber_avalanche_c', 'Elastic_Factory_evt_PoolCreated') }} AS p
        ON t.contract_address = p.pool
    {% if is_incremental() %}
        WHERE t.evt_block_time >= date_trunc('day', now() - interval '1 week')
    {% else %}
