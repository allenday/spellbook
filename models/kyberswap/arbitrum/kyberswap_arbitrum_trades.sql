...
kyberswap_dex AS (
    ...
    FROM {{ source('kyber_arbitrum', 'DMM_Pool_evt_Swap') }} AS t
    INNER JOIN {{ source('kyber_arbitrum', 'DMMFactory_evt_PoolCreated') }} AS p
    ...
    FROM {{ source('kyber_arbitrum', 'Elastic_Pool_evt_swap') }} AS t
    INNER JOIN {{ source('kyber_arbitrum', 'Elastic_Factory_evt_PoolCreated') }} AS p
        ON t.contract_address = p.pool
    {% if is_incremental() %}
        WHERE t.evt_block_time >= date_trunc('day', now() - interval '1 week')
    {% else %}
