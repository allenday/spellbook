...
SELECT 'bnb'                                                           AS blockchain,
       'biswap'                                                        AS project,
       '1'                                                             AS version,
       SAFE_CAST(TIMESTAMP_TRUNC(biswap_dex.block_time, DAY) AS date)      AS block_date,
       ...
       CAST(biswap_dex.token_bought_amount_raw AS BIGNUMERIC) AS token_bought_amount_raw,
       CAST(biswap_dex.token_sold_amount_raw AS BIGNUMERIC) AS token_sold_amount_raw,
       coalesce(
...
LEFT JOIN {{ source('prices', 'usd') }} p_bought
    ON p_bought.minute = TIMESTAMP_TRUNC(biswap_dex.block_time, minute)
...
LEFT JOIN {{ source('prices', 'usd') }} p_sold
    ON p_sold.minute = TIMESTAMP_TRUNC(biswap_dex.block_time, minute)
