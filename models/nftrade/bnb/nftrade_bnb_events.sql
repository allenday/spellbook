...
source_inventory as (
    SELECT
        ...
        CAST(udfs.bytea2numeric_v3(SUBSTRING(makerAssetData, 77, 64)) as BIGNUMERIC) as maker_id,
        ...
        CAST(udfs.bytea2numeric_v3(SUBSTRING(takerAssetData, 77, 64)) as BIGNUMERIC) as taker_id
    FROM
...
source_inventory_enriched as (
    ...
    SELECT
        ...
        TIMESTAMP_TRUNC(src.evt_block_time, day) as block_date,
        src.evt_block_number as block_number,
        src.token_id,
        nft_token.name as collection,
        CAST(src.amount_raw AS BIGNUMERIC) as amount_raw,
        src.amount_original,
        ...
        btx.from as tx_from,
        btx.to as tx_to,
        CAST(src.protocol_fees_raw AS FLOAT64) as platform_fee_amount_raw,
        CAST(src.protocol_fees AS FLOAT64) as platform_fee_amount,
        CAST(src.protocol_fees * p.price AS FLOAT64) as platform_fee_amount_usd,
        CAST(src.platform_fee_percentage AS FLOAT64) as platform_fee_percentage,
        CAST(src.royalty_fees_raw  AS FLOAT64) as royalty_fee_amount_raw,
        src.royalty_fees as royalty_fee_amount,
        src.royalty_fees * p.price as royalty_fee_amount_usd,
        CAST(src.royalty_fee_percentage AS FLOAT64) as royalty_fee_percentage,
    ...
    {{ source('prices','usd') }} p
        ON p.blockchain = 'bnb'
        AND p.minute = TIMESTAMP_TRUNC(src.evt_block_time, minute)
