...
, aggregated_punk_on_off_data as (
    select TIMESTAMP_TRUNC(a.evt_block_time, day) AS `day` 
            , a.punk_id 
            , listed_price
            , case when event_sub_type = 'Public Listing' then 'Active' else 'Not Listed' end as listed_bool
    from all_punk_events a 
    inner join (    select TIMESTAMP_TRUNC(evt_block_time, day) AS `day` 
                            , punk_id
                            , max(punk_event_index) as max_event
                    from all_punk_events
                    group by 1,2
                ) b -- max event per punk per day 
    on TIMESTAMP_TRUNC(a.evt_block_time, day) = b.day and a.punk_id = b.punk_id and a.punk_event_index = b.max_event
)
...
left join {{ source('prices', 'usd') }} p on p.minute = TIMESTAMP_TRUNC(e.day, minute)
