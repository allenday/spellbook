{{ config(
        ...
        partition_by = {"field": "evt_block_time_week"},
        materialized = 'view',
...
(
        ...
        (   select  a.from
                , a.to
                , a.evt_block_time
                , TIMESTAMP_TRUNC(a.evt_block_time, week) as evt_block_time_week
                , a.evt_block_number
                , a.evt_index
                , case when a.evt_tx_hash = lower('0x76d32b465ca332bbbe74f7a1834c6d354125f6950168c6123f8ab07440bc285e') and a.evt_index = 11 then 675
                        ...
                        when topic1 = '0x05af636b70da6819000c49f85b21fa82081c632069bb626f30932034099107d8' then cast(udfs.bytea2numeric_v3(SUBSTR(data, 3)) as int)
                        else cast(udfs.bytea2numeric_v3(SUBSTR(topic1, 3)) as int) end as punk_id
                , a.evt_tx_hash
        from {{ source('erc20_ethereum','evt_transfer') }} a
        inner join {{ source('ethereum','logs') }} b
                        on a.evt_tx_hash = b.tx_hash
                        {% if is_incremental() %}
                        and b.block_time >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 week')
                        {% endif %}
        where a.contract_address = lower('0xb47e3cd837dDF8e4c57F05d70Ab865de6e193BBB') -- cryptopunks contract
                ...
                {% if is_incremental() %} and a.evt_block_time >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 week') {% endif %}
        ...
        select  '0x0000000000000000000000000000000000000000' AS `from`
                , `to`
                , evt_block_time
                , TIMESTAMP_TRUNC(evt_block_time, week) as evt_block_time_week
                , evt_block_number
                , evt_index
                , punkIndex as punk_id
                , evt_tx_hash
        from {{ source('cryptopunks_ethereum','CryptoPunksMarket_evt_Assign') }}
        {% if is_incremental() %} where evt_block_time >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 week') {% endif %}
