...
with cryptopunks_bids_and_sales as (
    ...
    from
        (
            ...
            select
                "PunkBidWithdrawn" as event_type,
                punkindex as punk_id,
                cast(NULL as double) as sale_price,
                value / 1e18 as bid_amount,
                cast(NULL as varchar(5)) as to_address,
...
bid_sales as (
    select
        ...
        a.evt_block_time,
        a.evt_tx_hash
    from cryptopunks_bids_and_sales as a

    inner join {{ source('cryptopunks_ethereum','CryptoPunksMarket_evt_Transfer') }} as b
        on a.from_address = b.from and a.evt_block_number = b.evt_block_number and a.evt_index = (b.evt_index + 1)

    left outer join cryptopunks_bids_and_sales as c
...
regular_sales as (
    select
        ...
        case
            when a.toaddress = "0x83c8f28c26bf6aaca652df1dbbe0e1b56f8baba2" -- gem 
                then b.to
            else a.toaddress
        end as to_address,
        ...
        a.evt_block_time,
        a.evt_tx_hash
    from {{ source('cryptopunks_ethereum','CryptoPunksMarket_evt_PunkBought') }} as a
    left outer join {{ source('cryptopunks_ethereum','CryptoPunksMarket_evt_PunkTransfer') }} as b
        on
            a.punkindex = b.punkindex
...
left join {{ source('prices', 'usd') }} as p
    on
        p.minute = date_trunc("minute", a.evt_block_time)
...
left join {{ ref('nft_ethereum_aggregators') }} as agg on agg.contract_address = tx.to

where
    a.evt_tx_hash not in ("0x92488a00dfa0746c300c66a716e6cc11ba9c0f9d40d8c58e792cc7fcebf432d0" -- flash loan https://twitter.com/cryptopunksnfts/status/1453903818308083720
    )
