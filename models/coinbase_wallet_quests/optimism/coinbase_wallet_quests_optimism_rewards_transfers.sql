...
WITH distributions AS (
    ...
    INNER JOIN {{ ref('coinbase_wallet_quests_optimism_distributor_addresses') }} AS qa
        ON
            r.`from` = distributor_address
            AND r.contract_address = rewards_token

    WHERE
        evt_block_time >= cast('{{ project_start_date }}' AS timestamp) --arbitrary
        {% if is_incremental() %}
            ...
            AND 1 = (
                CASE
                    WHEN evt_block_time >= date_trunc('day', now() - interval '1 week') THEN 1
