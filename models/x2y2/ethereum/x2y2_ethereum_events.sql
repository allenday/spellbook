...
src_evt_inventory as (
    ...
    ,taker
    ,maker
    ,JSON_EXTRACT_SCALAR(inv.item, '$.data') as data
    ,udfs.bytea2numeric_v3(substring(JSON_EXTRACT_SCALAR(inv.item, '$.data'), 195,64)) as token_id
    ,'0x' || substring(JSON_EXTRACT_SCALAR(inv.item, '$.data'), 155, 40) as nft_contract_address
    ,JSON_EXTRACT_SCALAR(inv.detail, '$.executionDelegate') as execution_delegate
    ,JSON_EXTRACT_SCALAR(inv.item, '$.price') as price
    ,JSON_EXTRACT_SCALAR(inv.detail, '$.fees[0]') as fees_0
    ,JSON_EXTRACT_SCALAR(JSON_EXTRACT_SCALAR(inv.detail, '$.fees[0]'), '$.to') as fees_0_to
    ,JSON_EXTRACT_SCALAR(JSON_EXTRACT_SCALAR(inv.detail, '$.fees[0]'), '$.percentage') as platform_fee_percentage
    ...
    , COALESCE(JSON_EXTRACT_SCALAR(JSON_EXTRACT_SCALAR(inv.detail, '$.fees[0]'), '$.percentage'), 0)
        +COALESCE(JSON_EXTRACT_SCALAR(JSON_EXTRACT_SCALAR(inv.detail, '$.fees[1]'), '$.percentage'), 0)
        +COALESCE(JSON_EXTRACT_SCALAR(JSON_EXTRACT_SCALAR(inv.detail, '$.fees[2]'), '$.percentage'), 0)
        +COALESCE(JSON_EXTRACT_SCALAR(JSON_EXTRACT_SCALAR(inv.detail, '$.fees[3]'), '$.percentage'), 0)
...
, TIMESTAMP_TRUNC(prof.evt_block_time, day) AS block_date
, prof.evt_block_number AS block_number
, inv.token_id as token_id
, nft_token.name AS collection
, CAST(inv.price AS BIGNUMERIC) AS amount_raw
...
LEFT JOIN src_prices_usd pu
    ON pu.minute=TIMESTAMP_TRUNC(prof.evt_block_time, minute)
