...
src_evt_profit AS (
    SELECT
        *,
        COALESCE (currency = '0x0000000000000000000000000000000000000000', FALSE) AS is_native_eth
...
SELECT
    ...
    CASE
        WHEN (inv.fees_0 IS NULL OR inv.fees_0_to != '{{ fee_management_addr }}') AND (prof.evt_block_time < '2022-04-01' OR prof.evt_block_time >= '2022-05-01') THEN 'Private Sale'
    ...
    CASE
        WHEN prof.is_native_eth THEN 'ETH'
        ELSE currency_token.symbol
    END AS currency_symbol,
    CASE
        WHEN prof.is_native_eth THEN '{{ eth_erc20_addr }}'
        ELSE prof.currency
    ...
    et.to AS tx_to,
    CASE
        WHEN inv.fees_0_to = '{{ fee_management_addr }}' THEN ROUND(COALESCE(inv.price * inv.platform_fee_percentage / 1e6, 0), 0)
        ELSE 0
    END AS platform_fee_amount_raw,
    CASE
        WHEN inv.fees_0_to = '{{ fee_management_addr }}' THEN ROUND(COALESCE(inv.price * inv.platform_fee_percentage / 1e6, 0), 0) / POWER(10, currency_token.decimals)
        ELSE 0
    END AS platform_fee_amount,
    CASE
        WHEN inv.fees_0_to = '{{ fee_management_addr }}' THEN pu.price * ROUND(COALESCE(inv.price * inv.platform_fee_percentage / 1e6), 0) / POWER(10, currency_token.decimals)
        ELSE 0
    END AS platform_fee_amount_usd,
    CASE
        WHEN inv.fees_0_to = '{{ fee_management_addr }}' THEN COALESCE(inv.platform_fee_percentage / 1e4, 0)
        ELSE 0
    END AS platform_fee_percentage,
    CASE
        WHEN inv.fees_0_to = '{{ fee_management_addr }}' THEN COALESCE(inv.price * inv.royalty_fee_percentage / 1e6, 0)
        ELSE COALESCE(inv.price * inv.all_fee_percentage / 1e6, 0)
    END AS royalty_fee_amount_raw,
    CASE
        WHEN inv.fees_0_to = '{{ fee_management_addr }}' THEN COALESCE(inv.price * inv.royalty_fee_percentage / 1e6, 0) / POWER(10, currency_token.decimals)
        ELSE COALESCE(inv.price * inv.all_fee_percentage / 1e6, 0) / POWER(10, currency_token.decimals)
    END AS royalty_fee_amount,
    CASE
        WHEN inv.fees_0_to = '{{ fee_management_addr }}' THEN pu.price * COALESCE(inv.price * inv.royalty_fee_percentage / 1e6, 0) / POWER(10, currency_token.decimals)
    ...
    CASE
        WHEN inv.fees_0_to = '{{ fee_management_addr }}' THEN inv.royalty_fee_percentage / 1e4
    ...
    CASE
        WHEN inv.fees_0_to = '{{ fee_management_addr }}' THEN inv.fees_1_to
        ELSE inv.fees_0_to
...
FROM src_evt_profit AS prof
INNER JOIN src_evt_inventory AS inv
    ON
        inv.evt_block_time = prof.evt_block_time
        AND inv.itemhash = prof.itemhash
INNER JOIN src_eth_transactions AS et
    ON
        et.block_time = prof.evt_block_time
        AND et.hash = prof.evt_tx_hash
LEFT JOIN {{ ref('tokens_ethereum_nft') }} AS nft_token ON inv.nft_contract_address = nft_token.contract_address
LEFT JOIN {{ ref('tokens_ethereum_erc20') }} AS currency_token
    ON
        currency_token.contract_address = prof.currency
...
LEFT JOIN src_prices_usd AS pu
    ON
        pu.minute = date_trunc('minute', prof.evt_block_time)
        AND (
            pu.contract_address = prof.currency
