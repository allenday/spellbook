...
WITH sushiswap_dex AS (
    SELECT
        ...
        '' AS trace_address,
        t.evt_index
    FROM {{ source('sushiswap_gnosis', 'UniswapV2Pair_evt_Swap') }} AS t
    INNER JOIN {{ source('sushiswap_gnosis', 'UniswapV2Factory_evt_PairCreated') }} AS f
...
FROM sushiswap_dex
INNER JOIN {{ source('gnosis', 'transactions') }} AS tx
    ON
        sushiswap_dex.tx_hash = tx.hash
...
LEFT JOIN {{ ref('tokens_erc20') }} AS erc20a
    ON
        erc20a.contract_address = sushiswap_dex.token_bought_address
        AND erc20a.blockchain = 'gnosis'
LEFT JOIN {{ ref('tokens_erc20') }} AS erc20b
    ON
        erc20b.contract_address = sushiswap_dex.token_sold_address
        AND erc20b.blockchain = 'gnosis'
LEFT JOIN {{ source('prices', 'usd') }} AS p_bought
    ON
        p_bought.minute = date_trunc('minute', sushiswap_dex.block_time)
...
LEFT JOIN {{ source('prices', 'usd') }} AS p_sold
    ON
        p_sold.minute = date_trunc('minute', sushiswap_dex.block_time)
