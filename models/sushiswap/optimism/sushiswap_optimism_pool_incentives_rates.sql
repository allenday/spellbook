...
, joined as (
        SELECT TIMESTAMP_TRUNC(e.evt_block_time, day)                                                            AS block_date
             , e.evt_block_number
             , e.evt_block_time
...
SELECT block_date
     ...
     , cast(tokens_per_second_raw * alloc_point_share as FLOAT64) /
       cast(power(10, decimals) as FLOAT64)                                                             AS alloc_tokens_per_second
     , symbol                                                                                          AS reward_token_symbol
     , decimals                                                                                        AS reward_token_decimals
FROM (
        SELECT *
        , cast(alloc_points as FLOAT64) / cast(total_alloc_points as FLOAT64) AS alloc_point_share
        FROM joined
) a
