...
pools AS (
    SELECT
        ...
        evt_index,
        contract_address,
        rewarder AS rewarder_address,
        pid,
        allocpoint AS alloc_points
...
pool_updates AS (
    ...
    FROM pools AS p
    LEFT JOIN {{ ref('sushiswap_optimism_pool_incentives_config') }} AS c
        ON p.rewarder_address = c.contract_address
    LEFT JOIN {{ ref('sushiswap_optimism_pool_incentives_mappings') }} AS m
        ON
            p.contract_address = m.contract_address
            AND p.pid = m.pid
),
...
joined AS (
    ...
    FROM events AS e
    FULL OUTER JOIN pool_updates AS pu
        ON
            e.evt_block_number >= pu.evt_block_number
            AND e.evt_block_number < pu.next_evt_number
    FULL OUTER JOIN rates_updates AS ru
        ON
            e.evt_block_number >= ru.evt_block_number
