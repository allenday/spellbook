...
WITH dexs AS (
    ...
    SELECT
        ...
        t.evt_index
    FROM
        {{ source('arbswap_arbitrum', 'SwapPair_evt_Swap') }} AS t
    INNER JOIN {{ source('arbswap_arbitrum', 'SwapFactory_evt_PairCreated') }} AS f
    ...
    SELECT
        ...
        t.evt_index
    FROM
        {{ source('arbswap_arbitrum', 'ArbswapStableSwapTwoPool_evt_TokenExchange') }} AS t
    INNER JOIN {{ source('arbswap_arbitrum', 'ArbswapStableSwapFactory_evt_NewStableSwapPair') }} AS f
