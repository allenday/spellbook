...
WITH dexs AS (
    ...
    SELECT
        ...
        CASE WHEN bought_id = '0' THEN f.tokenb ELSE f.tokena END AS token_sold_address,
    ...
    INNER JOIN (
        SELECT a.*
        FROM {{ source('pancakeswap_v2_bnb', 'PancakeStableSwapFactory_evt_NewStableSwapPair') }} AS a
        INNER JOIN (
            SELECT
                swapcontract,
                MAX(evt_block_time) AS latest_time
        ...
        ON
            t.contract_address = f.swapcontract
            {% if is_incremental() %}
