...
WITH events AS (
    ...
    SELECT
        ...
        events.version,
        events.block_time,
        TIMESTAMP_TRUNC(events.block_time, day) AS block_date,
        events.token_id,
        ...
        CAST(events.number_of_items AS BIGNUMERIC) AS number_of_items,
        events.trade_category,
        'Trade' AS evt_type,
        events.seller,
        events.buyer,
        events.amount_raw/POWER(10, bnb_bep20_tokens.decimals) AS amount_original,
        CAST(events.amount_raw AS BIGNUMERIC) AS amount_raw,
        ...
        bt.from AS tx_from,
        bt.to AS tx_to,
        CAST(amount_raw * platform_fee_percentage AS FLOAT64) AS platform_fee_amount_raw,
        ...
        CAST(100 * platform_fee_percentage AS FLOAT64) AS platform_fee_percentage,
        CAST(amount_raw * royalty_fee_percentage AS FLOAT64) AS royalty_fee_amount_raw,
        ...
        CAST(100*royalty_fee_percentage AS FLOAT64) AS royalty_fee_percentage,
    ...
    LEFT JOIN {{ source('prices', 'usd') }} prices ON prices.minute=TIMESTAMP_TRUNC(events.block_time, minute)
