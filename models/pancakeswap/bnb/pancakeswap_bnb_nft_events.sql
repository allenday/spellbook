...
FROM events
LEFT JOIN {{ ref('nft_aggregators') }} AS agg ON events.buyer = agg.contract_address AND agg.blockchain = 'bnb'
LEFT JOIN {{ ref('tokens_erc20') }} AS bnb_bep20_tokens ON bnb_bep20_tokens.contract_address = events.currency_contract AND bnb_bep20_tokens.blockchain = 'bnb'
LEFT JOIN {{ ref('tokens_nft') }} AS bnb_nft_tokens ON bnb_nft_tokens.contract_address = events.currency_contract AND bnb_nft_tokens.blockchain = 'bnb'
LEFT JOIN {{ source('prices', 'usd') }} AS prices
    ON
        prices.minute = date_trunc('minute', events.block_time)
        ...
        {% endif %}
INNER JOIN {{ source('bnb','transactions') }} AS bt
    ON
        bt.hash = events.tx_hash
