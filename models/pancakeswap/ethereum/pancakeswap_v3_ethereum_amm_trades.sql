...
{% set project_start_date = '2023-03-31' %}
...
WITH dexs AS (
    ...
    SELECT
        ...
        t.evt_index
    FROM
        {{ source('pancakeswap_v3_ethereum', 'PancakeV3Pool_evt_Swap') }} AS t
    INNER JOIN
        {{ source('pancakeswap_v3_ethereum', 'PancakeV3Factory_evt_PoolCreated') }} AS f
...
FROM dexs
INNER JOIN
    {{ source('ethereum', 'transactions') }} AS tx
    ON
