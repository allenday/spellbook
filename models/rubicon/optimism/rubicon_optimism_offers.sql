...
(
    ...
    AND e.evt_block_time >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
),
...
(
    ...
    SELECT
        ...
        CAST(t.take_amt AS BIGNUMERIC) AS sold_amount_raw,
        CAST(t.give_amt AS BIGNUMERIC) AS bought_amount_raw,
        erc20_sell.symbol AS sell_token_symbol,
        erc20_buy.symbol AS buy_token_symbol,
        CAST(t.take_amt AS BIGNUMERIC) / power(10, erc20_sell.decimals) AS sold_amount,
        CAST(t.give_amt AS BIGNUMERIC) / power(10, erc20_buy.decimals) AS bought_amount,
        (CAST(t.take_amt AS BIGNUMERIC) / power(10, erc20_sell.decimals)) * sell_token_price.price AS sold_amount_usd,
        (CAST(t.give_amt AS BIGNUMERIC) / power(10, erc20_buy.decimals)) * buy_token_price.price AS bought_amount_usd
    ...
    LEFT JOIN {{ source('prices', 'usd') }} sell_token_price
        ON sell_token_price.minute = TIMESTAMP_TRUNC(t.evt_block_time, minute)
    ...
    LEFT JOIN {{ source('prices', 'usd') }} buy_token_price
        ON buy_token_price.minute = TIMESTAMP_TRUNC(t.evt_block_time, minute)
...
SELECT 
    ...
    'rubicon' AS project,
    '1' AS version,
    CAST(TIMESTAMP_TRUNC(offers.block_time, DAY) AS date) AS block_date,
    offers.block_time,
    offers.block_number,
    ...
    CAST(offers.sell_amount_raw AS BIGNUMERIC) AS sell_amount_raw,
    CAST(offers.buy_amount_raw AS BIGNUMERIC) AS buy_amount_raw,
    trades.sold_amount AS sold_amount,
    trades.bought_amount AS bought_amount,
    CAST(trades.sold_amount_raw AS BIGNUMERIC) AS sold_amount_raw,
    CAST(trades.bought_amount_raw AS BIGNUMERIC) AS bought_amount_raw,
    ...
    ((CAST(txn.gas_used AS BIGNUMERIC) / power(10, 18)) * CAST(txn.gas_price AS BIGNUMERIC)) + ((CAST(txn.l1_gas_used AS BIGNUMERIC) / power(10, 18)) * CAST(txn.l1_gas_price AS BIGNUMERIC) * CAST(txn.l1_fee_scalar AS BIGNUMERIC)) AS txn_cost_eth,
    eth.price AS eth_price,
    ((CAST(txn.gas_used AS BIGNUMERIC) / power(10, 18)) * CAST(txn.gas_price AS BIGNUMERIC)) + ((CAST(txn.l1_gas_used AS BIGNUMERIC) / power(10, 18)) * CAST(txn.l1_gas_price AS BIGNUMERIC) * CAST(txn.l1_fee_scalar AS BIGNUMERIC)) * eth.price AS txn_cost_usd,
...
INNER JOIN {{ source('optimism', 'transactions') }} txn
    ...
    {% endif %}
    {% if is_incremental() %} -- only run this filter if it is an incremental run
    AND txn.block_time >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
...
LEFT JOIN {{ source('prices', 'usd') }} sell_token_price
    ON sell_token_price.minute = TIMESTAMP_TRUNC(offers.block_time, minute)
    ...
    {% endif %}
    {% if is_incremental() %}
    AND sell_token_price.minute >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
    
-- get the price data for the buy token
LEFT JOIN {{ source('prices', 'usd') }} buy_token_price
    ON buy_token_price.minute = TIMESTAMP_TRUNC(offers.block_time, minute)
    ...
    {% endif %}
    {% if is_incremental() %}
    AND buy_token_price.minute >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}

-- get the price of eth at the time of the offer 
LEFT JOIN {{ source('prices', 'usd') }}  eth
    ON eth.minute = TIMESTAMP_TRUNC(offers.block_time, minute)
    AND cast(eth.contract_address AS STRING) = '0xdeaddeaddeaddeaddeaddeaddeaddeaddead0000' -- this is for optimism specifically
    AND eth.blockchain = 'optimism'
    {% if not is_incremental() %}
    AND eth.minute >= cast('{{ project_start_date }}' AS timestamp)
    {% endif %}
    {% if is_incremental() %}
    AND eth.minute >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
