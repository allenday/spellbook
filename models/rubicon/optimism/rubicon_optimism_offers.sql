...
WITH offers AS (
    ...
    SELECT
        ...
        e.maker,
        e.id AS offer_id
    FROM {{ source('rubicon_optimism', 'RubiconMarket_evt_LogMake') }} AS e

    -- filter out offers that were created before the project start date
    WHERE
        e.evt_block_time >= cast('{{ project_start_date }}' AS timestamp)
...
trades AS (
    ...
    FROM {{ source('rubicon_optimism', 'RubiconMarket_evt_LogTake') }} AS t

    -- get the relevant sell token data
    LEFT JOIN {{ ref('tokens_erc20') }} AS erc20_sell
        ON
            erc20_sell.contract_address = t.pay_gem
            AND erc20_sell.blockchain = 'optimism'

    -- get the relevant buy token data
    LEFT JOIN {{ ref('tokens_erc20') }} AS erc20_buy
        ON
            erc20_buy.contract_address = t.buy_gem
            AND erc20_buy.blockchain = 'optimism'

    -- get the sell token price
    LEFT JOIN {{ source('prices', 'usd') }} AS sell_token_price
        ON
            sell_token_price.minute = date_trunc('minute', t.evt_block_time)
    ...
    LEFT JOIN {{ source('prices', 'usd') }} AS buy_token_price
        ON
            buy_token_price.minute = date_trunc('minute', t.evt_block_time)
    ...
    WHERE
        t.evt_block_time >= cast('{{ project_start_date }}' AS timestamp)
        {% if is_incremental() %} -- only run this filter if it is an incremental run
...
INNER JOIN {{ source('optimism', 'transactions') }} AS txn
    ON
        txn.hash = offers.tx_hash
...
LEFT JOIN {{ ref('tokens_erc20') }} AS erc20_sell
    ON
        erc20_sell.contract_address = offers.sell_token_address
        AND erc20_sell.blockchain = 'optimism'

-- get the relevant buy token data
LEFT JOIN {{ ref('tokens_erc20') }} AS erc20_buy
    ON
        erc20_buy.contract_address = offers.buy_token_address
        AND erc20_buy.blockchain = 'optimism'

-- get the price data for the sell token
LEFT JOIN {{ source('prices', 'usd') }} AS sell_token_price
    ON
        sell_token_price.minute = date_trunc('minute', offers.block_time)
...
LEFT JOIN {{ source('prices', 'usd') }} AS buy_token_price
    ON
        buy_token_price.minute = date_trunc('minute', offers.block_time)
...
LEFT JOIN {{ source('prices', 'usd') }} AS eth
    ON
        eth.minute = date_trunc('minute', offers.block_time)
...
LEFT JOIN trades_grouped AS trades
