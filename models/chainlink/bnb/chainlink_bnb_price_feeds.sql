...
FROM
    (
        ...
        FROM {{ source('bnb', 'logs') }} AS l
        INNER JOIN {{ ref('chainlink_bnb_oracle_addresses') }} AS cfa ON l.contract_address = cfa.aggregator_address
        WHERE
...
LEFT JOIN {{ ref('chainlink_bnb_oracle_token_mapping') }} AS o ON c.proxy_address = o.proxy_address
