...
FROM
    (
        ...
        FROM {{ source('polygon', 'logs') }} AS l
        INNER JOIN {{ ref('chainlink_polygon_oracle_addresses') }} AS cfa ON l.contract_address = cfa.aggregator_address
        WHERE
            l.topic1 = '{{ answer_updated }}'
...
LEFT JOIN {{ ref('chainlink_polygon_oracle_token_mapping') }} AS o ON c.proxy_address = o.proxy_address
