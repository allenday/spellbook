...
FROM (
    ...
    FROM {{ source('hop_protocol_optimism', 'L2_OptimismBridge_evt_TransferSent') }} AS ts
    {% if is_incremental() %}
    ...
    FROM {{ source ('hop_protocol_optimism', 'L2_OptimismBridge_evt_TransferFromL1Completed') }} AS tl
    {% if is_incremental() %}
        WHERE evt_block_time >= (NOW() - interval '14 days')
    {% endif %}
    ...
    SELECT
        ...
        END
            AS source_chain_id,
        (SELECT chain_id FROM {{ ref('chain_ids') }} WHERE lower(chain_name) = 'optimism') AS destination_chain_id
    FROM {{ source ('hop_protocol_optimism', 'L2_OptimismBridge_evt_WithdrawalBonded') }} AS wb
    LEFT JOIN {{ source ('hop_protocol_arbitrum' ,'L2_ArbitrumBridge_evt_TransferSent') }} AS arb
        ON
            arb.evt_block_time BETWEEN (wb.evt_block_time - interval '30 days') AND (wb.evt_block_time + interval '1 day') --usually < ~20 mins, but extending longer for safety & OP blocktimestamp fix (used to have ~15 min delay)
            AND arb.transferid = wb.transferid
            {% if is_incremental() %}
                AND arb.evt_block_time >= (NOW() - interval '45 days')
            {% endif %}
    LEFT JOIN {{ source ('hop_protocol_polygon', 'L2_PolygonBridge_evt_TransferSent') }} AS poly
        ON
            poly.evt_block_time BETWEEN (wb.evt_block_time - interval '30 days') AND (wb.evt_block_time + interval '1 day') --usually < ~20 mins, but extending longer for safety & OP blocktimestamp fix (used to have ~15 min delay)
            AND poly.transferid = wb.transferid
            {% if is_incremental() %}
                AND poly.evt_block_time >= (NOW() - interval '45 days')
            {% endif %}
    LEFT JOIN {{ source ('hop_protocol_gnosis', 'L2_xDaiBridge_evt_TransferSent') }} AS gno
        ON
            gno.evt_block_time BETWEEN (wb.evt_block_time - interval '30 days') AND (wb.evt_block_time + interval '1 day') --usually < ~20 mins, but extending longer for safety & OP blocktimestamp fix (used to have ~15 min delay)
...
LEFT JOIN {{ ref('hop_protocol_addresses') }} AS hba
    ON
        tf.project_contract_address = hba.`l2Bridge`
        AND tf.block_number >= hba.bridgedeployedblocknumber
        AND hba.blockchain = 'optimism'
LEFT JOIN {{ source('optimism', 'transactions') }} AS t
    ON
        t.block_time = tf.block_time
...
LEFT JOIN {{ ref('chain_ids') }} AS cid_source
    ON cid_source.chain_id = tf.source_chain_id
LEFT JOIN {{ ref('chain_ids') }} AS cid_dest
