...
, '' as version
, tf.block_time
, TIMESTAMP_TRUNC(tf.block_time, day) AS block_date
, tf.block_number
, tx_hash
, '' as sender
...
FROM (
    ...
    FROM {{ source ('hop_protocol_optimism', 'L2_OptimismBridge_evt_WithdrawalBonded') }} wb
        LEFT JOIN {{ source ('hop_protocol_arbitrum' ,'L2_ArbitrumBridge_evt_TransferSent') }} arb
            ...
            {% if is_incremental() %}
            AND arb.evt_block_time >= (CURRENT_TIMESTAMP() - interval '45 days')
              {% endif %}
        LEFT JOIN {{ source ('hop_protocol_polygon', 'L2_PolygonBridge_evt_TransferSent') }} poly
            ...
            {% if is_incremental() %}
            AND poly.evt_block_time >= (CURRENT_TIMESTAMP() - interval '45 days')
              {% endif %}
        LEFT JOIN {{ source ('hop_protocol_gnosis', 'L2_xDaiBridge_evt_TransferSent') }} gno
            ...
            {% if is_incremental() %}
            AND gno.evt_block_time >= (CURRENT_TIMESTAMP() - interval '45 days')
              {% endif %}
    {% if is_incremental() %}
    WHERE wb.evt_block_time >= (CURRENT_TIMESTAMP() - interval '14 days')
    {% endif %}
