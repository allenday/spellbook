...
WITH dexs AS (
    ...
    FROM
        {{ source('swapr_ethereum', 'DXswapPair_evt_Swap') }} AS t
    INNER JOIN {{ source('swapr_ethereum', 'DXswapFactory_evt_PairCreated') }} AS f ON f.pair = t.contract_address
    {% if is_incremental() %}
        WHERE
...
LEFT JOIN {{ source('prices', 'usd') }} AS pa
    ON
        pa.minute = date_trunc('minute', dexs.block_time)
...
LEFT JOIN {{ source('prices', 'usd') }} AS pb
    ON
        pb.minute = date_trunc('minute', dexs.block_time)
