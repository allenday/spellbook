...
SELECT
    ...
    CASE
        WHEN type = 'Legacy' THEN (cast(gas_price AS double) / 1e18 * cast(txns.gas_used AS double))
    ...
    CASE
        WHEN type = 'Legacy' THEN (cast(gas_price AS double) / 1e18 * cast(txns.gas_used AS double)) * p.price
...
FROM {{ source('ethereum','transactions') }} AS txns
INNER JOIN {{ source('ethereum','blocks') }} AS blocks
    ON
