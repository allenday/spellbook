...
SELECT 
     ...
     CASE WHEN type = 'Legacy' THEN (cast(gas_price as FLOAT64)/1e18 * cast(txns.gas_used as FLOAT64))
          WHEN type = 'AccessList' THEN (cast(gas_price as FLOAT64)/1e18 * cast(txns.gas_used as FLOAT64))
          WHEN type = 'DynamicFee' THEN ((cast(base_fee_per_gas as FLOAT64)/1e18 + cast(priority_fee_per_gas as FLOAT64)/1e18)* cast(txns.gas_used as FLOAT64)) 
          END AS tx_fee_native, 
     CASE WHEN type = 'Legacy' THEN (cast(gas_price as FLOAT64)/1e18 * cast(txns.gas_used as FLOAT64)) * p.price 
          WHEN type = 'AccessList' THEN (cast(gas_price as FLOAT64)/1e18 * cast(txns.gas_used as FLOAT64)) * p.price 
          WHEN type = 'DynamicFee' THEN ((cast(base_fee_per_gas as FLOAT64)/1e18 + cast(priority_fee_per_gas as FLOAT64)/1e18)* cast(txns.gas_used as FLOAT64))  * p.price 
          END AS tx_fee_usd,
