...
SELECT
    ...
    CASE
        WHEN block_number >= 13082000 AND txns.to = '0x0000000000000000000000000000000000001000' THEN value / 1e18 * 10 / 100
        ELSE NULL::double
    END AS burned_native, -- change after BEP95
    CASE
        WHEN block_number >= 13082000 AND txns.to = '0x0000000000000000000000000000000000001000' THEN value / 1e18 * 10 / 100 * p.price
        ELSE NULL::double
    END AS burned_usd, -- change after BEP95
    miner AS validator,
...
FROM {{ source('bnb','transactions') }} AS txns
INNER JOIN {{ source('bnb','blocks') }} AS blocks
    ON
