...
SELECT
    ...
    CASE
        WHEN type = 'Legacy' THEN (gas_price * txns.gas_used) / 1e18
        WHEN type = 'DynamicFee' THEN ((base_fee_per_gas + priority_fee_per_gas) * txns.gas_used) / 1e18
    END AS tx_fee_native,
    CASE
        WHEN type = 'Legacy' THEN (gas_price * txns.gas_used) / 1e18 * p.price
...
INNER JOIN {{ source('avalanche_c','blocks') }} AS blocks
    ON
