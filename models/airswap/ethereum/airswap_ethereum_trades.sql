...
WITH dexs AS (
    SELECT
        ...
        evt_index
    FROM {{ source('airswap_ethereum', 'Light_evt_Swap') }} AS e
    {% if is_incremental() %}
    ...
    SELECT
        ...
        cast(NULL AS double) AS amount_usd,
        evt_index
    FROM {{ source('airswap_ethereum', 'Light_v0_evt_Swap') }} AS e
    {% if is_incremental() %}
        WHERE evt_block_time >= date_trunc('day', now() - interval '1 week')
    {% endif %}
    ...
    SELECT
        ...
        cast(NULL AS double) AS amount_usd,
        evt_index
    FROM {{ source('airswap_ethereum', 'swap_evt_Swap') }} AS e
    {% if is_incremental() %}
        WHERE evt_block_time >= date_trunc('day', now() - interval '1 week')
    {% endif %}
    ...
    SELECT
        ...
        cast(NULL AS double) AS amount_usd,
        evt_index
    FROM {{ source('airswap_ethereum', 'Swap_v3_evt_Swap') }} AS e
    {% if is_incremental() %}
        WHERE evt_block_time >= date_trunc('day', now() - interval '1 week')
    {% endif %}
)
