...
SELECT
    ...
    CASE
        WHEN t.evt_block_time >= (SELECT min_hour FROM price_bounds) AND t.evt_block_time <= (SELECT max_hour FROM price_bounds) THEN CAST(pu.median_price * t.amount / POWER(10, 18) AS DOUBLE)
    ...
    t.evt_index
FROM {{ source('velodrome_optimism', 'MerkleClaim_evt_Claim') }} AS t
LEFT JOIN {{ ref('dex_prices') }} AS pu
    ON
