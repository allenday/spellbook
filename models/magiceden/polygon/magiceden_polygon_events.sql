...
WITH trades AS (
    SELECT
        CASE WHEN direction = 0 THEN 'buy' ELSE 'sell' END AS trade_category,
        evt_block_time,
        evt_block_number,
        evt_tx_hash,
        contract_address,
    ...
    WHERE
        substring(nonce, 1, 38) = '{{ magic_eden_nonce }}'
...
native_order_return_amount AS (
    ...
    FROM (
        ...
        FROM trades AS t
        INNER JOIN {{ source('polygon', 'traces') }} AS tc
            ON
                t.evt_block_number = tc.block_number
...
native_order_total_amount AS (
    ...
    FROM native_order_summary AS o
    INNER JOIN {{ source('polygon', 'transactions') }} AS t
        ON
            o.evt_block_number = t.block_number
        ...
        {% endif %}
            ...
            {% endif %}
    LEFT JOIN native_order_return_amount
        AS r ON o.evt_block_number = r.evt_block_number
...
native_trade_amount_summary AS (
    SELECT
        ...
        (CASE
            WHEN 1.0 - o.order_amount_percentage >= 0.02 THEN cast(t.amount_raw AS decimal(38, 0)) / o.order_amount_percentage * 0.02
        ...
        (CASE
            WHEN 1.0 - o.order_amount_percentage >= 0.025 THEN cast(t.amount_raw AS decimal(38, 0)) / o.order_amount_percentage * 0.005
    ...
    FROM trades AS t
    INNER JOIN native_order_total_amount AS o
        ON
...
erc20_trade_amount_detail AS (
    ...
    FROM {{ source('erc20_polygon', 'evt_transfer') }} AS e
    INNER JOIN {{ source('polygon', 'transactions') }} AS tx
        ON
            e.evt_block_number = tx.block_number
    ...
    INNER JOIN trades AS t
        ON
            e.evt_block_number = t.evt_block_number AND t.currency_contract = e.contract_address
        ...
        {% endif %}
            ...
            {% endif %}
    WHERE
        t.original_erc20_token NOT IN ('0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee', '0x0000000000000000000000000000000000001010')
...
INNER JOIN {{ source('polygon','transactions') }} AS t
    ...
    {% endif %}
        {% if is_incremental() %}
            AND t.block_time >= date_trunc('day', now() - interval '1 week')
        {% endif %}
LEFT JOIN trade_amount_summary AS s
    ON
        a.evt_block_number = s.evt_block_number
        AND a.evt_tx_hash = s.evt_tx_hash
        AND a.token_id = s.token_id
        AND a.currency_contract = s.currency_contract
LEFT JOIN {{ ref('tokens_erc20') }} AS erc ON erc.blockchain = 'polygon' AND erc.contract_address = a.currency_contract
LEFT JOIN {{ source('prices', 'usd') }} AS p
    ON
        p.contract_address = a.currency_contract
