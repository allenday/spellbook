...
-- Max platform fee percentage is set to MOD( 2, and) max royalty fee percentage is set to MOD( 0.5, .)
-- This can be updated if a better solution is found.

native_order_summary AS (
    SELECT evt_block_number,
        evt_tx_hash,
        sum(CAST(amount_raw AS BIGNUMERIC)) AS order_amount_raw
    FROM trades
...
native_order_return_amount AS (
    ...
    FROM (
        ...
        INNER JOIN {{ source('polygon', 'traces') }} tc ON t.evt_block_number = tc.block_number
            ...
            {% endif %}
            {% if is_incremental() %}
            AND tc.block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
            {% endif %}
        WHERE tc.`to` = t.buyer
    ) t
    GROUP BY 1, 2
...
SELECT
  ...
  CAST(s.amount_raw as BIGNUMERIC) AS amount_raw,
  ...
  CAST(s.platform_fee_amount_raw / power(10, erc.decimals) AS FLOAT64) AS platform_fee_amount,
  CAST(s.platform_fee_amount_raw / power(10, erc.decimals) * p.price AS FLOAT64) AS platform_fee_amount_usd,
  CAST(s.platform_fee_amount_raw  / s.amount_raw * 100 as FLOAT64) as platform_fee_percentage,
  CAST(s.royalty_fee_amount_raw AS FLOAT64) AS royalty_fee_amount_raw,
  CAST(s.royalty_fee_amount_raw / power(10, erc.decimals) AS FLOAT64) AS royalty_fee_amount,
  CAST(s.royalty_fee_amount_raw / power(10, erc.decimals) * p.price AS FLOAT64) AS royalty_fee_amount_usd,
  CAST(s.royalty_fee_amount_raw / s.amount_raw * 100 AS FLOAT64) AS royalty_fee_percentage,
...
LEFT JOIN {{ source('prices', 'usd') }} p ON p.contract_address = a.currency_contract
    AND p.minute = TIMESTAMP_TRUNC(a.evt_block_time, minute)
    {% if not is_incremental() %}
