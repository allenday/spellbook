...
SELECT
  ...
  block_date,
  block_time,
  CAST(block_slot AS STRING) as block_number,
  ...
  CAST(abs(post_balances[0] - pre_balances[0]) AS STRING) AS amount_raw,
  ...
  CASE WHEN (array_contains(account_keys, 'M2mx93ekt1fmXSVkTrUL9xVFHkmME8HTUi5Cyc5aF7K'))
         AND array_contains(log_messages, 'Program log: Instruction: ExecuteSale')
         AND array_contains(log_messages, 'Program log: Instruction: Buy') THEN CAST(instructions[1].account_arguments[2] AS STRING)
       WHEN (array_contains(account_keys, 'CMZYPASGWeTz7RNGHaRJfCq2XQ5pYK6nDvVQxzkH51zb'))
         AND array_contains(log_messages, 'Program log: Instruction: SetAuthority') THEN CAST(COALESCE(instructions[6].account_arguments[9], instructions[5].account_arguments[9],
         instructions[4].account_arguments[9], instructions[2].account_arguments[7], instructions[1].account_arguments[10], instructions[0].account_arguments[10]) AS STRING)
       END AS token_id,
  ...
  CASE WHEN (array_contains(account_keys, 'M2mx93ekt1fmXSVkTrUL9xVFHkmME8HTUi5Cyc5aF7K'))
         AND array_contains(log_messages, 'Program log: Instruction: ExecuteSale')
         AND array_contains(log_messages, 'Program log: Instruction: Buy') THEN CAST(instructions[2].account_arguments[1] AS STRING)
  ...
  2*abs(post_balances[0] - pre_balances[0])/100 as platform_fee_amount_raw,
  ...
  signatures[0] || '-' || id || '-' || CAST(instructions[0] AS STRING) as unique_trade_id,
  instructions,
  signatures,
  log_messages
