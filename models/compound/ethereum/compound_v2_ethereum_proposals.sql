...
from {{ source('compound_v2_ethereum', 'GovernorBravoDelegate_evt_ProposalCreated') }} as pcr
left join cte_sum_votes as csv on csv.proposalid = pcr.id
left join {{ source('compound_v2_ethereum', 'GovernorBravoDelegate_evt_ProposalCanceled') }} as pca on pca.id = pcr.id
left join {{ source('compound_v2_ethereum', 'GovernorBravoDelegate_evt_ProposalExecuted') }} as pex on pex.id = pcr.id
left join {{ source('compound_v2_ethereum', 'GovernorBravoDelegate_evt_ProposalQueued') }} as pqu on pex.id = pcr.id
{% if is_incremental() %}
