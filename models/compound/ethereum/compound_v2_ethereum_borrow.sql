...
with borrows as (
    ...
    left join {{ source('prices', 'usd') }} as p
        on
            p.minute = date_trunc('minute', evt_borrow.evt_block_time)
...
repays as (
    ...
    left join {{ source('prices', 'usd') }} as p
        on
            p.minute = date_trunc('minute', evt_repay.evt_block_time)
            and p.contract_address = ctokens.asset_address
            and p.blockchain = 'ethereum'
            {% if is_incremental() %}
                and p.minute >= date_trunc('day', now() - interval '1 week')
...
liquidations as (
    ...
    left join {{ source('prices', 'usd') }} as p
        on
            p.minute = date_trunc('minute', evt_liquidate.evt_block_time)
            and p.contract_address = ctokens.asset_address
            and p.blockchain = 'ethereum'
            {% if is_incremental() %}
                and p.minute >= date_trunc('day', now() - interval '1 week')
