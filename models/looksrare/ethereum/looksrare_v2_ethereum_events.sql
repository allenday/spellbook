...
WITH looksrare_v2_trades AS (
    ...
    FROM {{ source('looksrare_v2_ethereum','LooksRareProtocol_evt_TakerAsk') }} AS l
    {% if is_incremental() %}
    ...
    SELECT
        ...
        CASE WHEN l.feerecipients[1] != '0x0000000000000000000000000000000000000000' THEN l.feerecipients[1] END AS royalty_fee_receive_address,
        get_json_object(l.nonceinvalidationparameters, '$.orderHash') AS order_hash
    FROM {{ source('looksrare_v2_ethereum','LooksRareProtocol_evt_TakerBid') }} AS l
    {% if is_incremental() %}
        WHERE l.evt_block_time >= date_trunc('day', now() - interval '1 week')
    {% endif %}
    {% if not is_incremental() %}
...
SELECT
    ...
    CASE
        WHEN lt.currency = '0x0000000000000000000000000000000000000000' THEN lt.amount_raw / POWER(10, 18)
        ELSE lt.amount_raw / POWER(10, pu.decimals)
    END AS amount_original,
    CASE
        WHEN lt.currency = '0x0000000000000000000000000000000000000000' THEN pu.price * lt.amount_raw / POWER(10, 18)
    ...
    lt.number_of_items,
    CASE
        WHEN lt.currency = '0x0000000000000000000000000000000000000000' THEN '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
        ELSE lt.currency
    ...
    CASE
        WHEN lt.currency = '0x0000000000000000000000000000000000000000' THEN lt.platform_fee_amount_raw / POWER(10, 18)
        ELSE lt.platform_fee_amount_raw / POWER(10, currency_tok.decimals)
    END AS platform_fee_amount,
    CASE
        WHEN lt.currency = '0x0000000000000000000000000000000000000000' THEN pu.price * lt.platform_fee_amount_raw / POWER(10, 18)
    ...
    CASE
        WHEN lt.currency = '0x0000000000000000000000000000000000000000' THEN lt.royalty_fee_amount_raw / POWER(10, 18)
        ELSE lt.royalty_fee_amount_raw / POWER(10, currency_tok.decimals)
    END AS royalty_fee_amount,
    CASE
        WHEN lt.currency = '0x0000000000000000000000000000000000000000' THEN pu.price * lt.royalty_fee_amount_raw / POWER(10, 18)
        ELSE pu.price * lt.royalty_fee_amount_raw / POWER(10, currency_tok.decimals)
    END AS royalty_fee_amount_usd,
    CASE
        WHEN lt.royalty_fee_receive_address IS NOT NULL AND lt.currency = '0x0000000000000000000000000000000000000000' THEN 'ETH'
...
FROM looksrare_v2_trades AS lt
INNER JOIN {{ source('ethereum','transactions') }} AS et
    ON
        et.block_number = lt.block_number
...
{% if not is_incremental() %}
    ...
    {% endif %}
LEFT JOIN {{ ref('tokens_ethereum_nft') }} AS nft ON lt.nft_contract_address = nft.contract_address
...
LEFT JOIN {{ ref('tokens_ethereum_erc20') }} AS currency_tok ON lt.currency = currency_tok.contract_address
LEFT JOIN {{ ref('prices_usd_forward_fill') }} AS pu
    ON
        pu.blockchain = 'ethereum'
        AND pu.minute = date_trunc('minute', lt.block_time)
        AND (
            lt.currency = pu.contract_address
