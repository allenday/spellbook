...
WITH looksrare_v2_trades AS (
    ...
    {% if is_incremental() %}
    WHERE l.evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
...
, 'v2' AS version
, lt.block_time
, TIMESTAMP_TRUNC(lt.block_time, day) AS block_date
, lt.block_number
, lt.trade_category
, 'Trade' AS evt_type
, lt.seller
, lt.buyer
, CAST(lt.amount_raw as BIGNUMERIC) as amount_raw
...
LEFT JOIN {{ ref('prices_usd_forward_fill') }} pu ON pu.blockchain='ethereum'
    AND pu.minute=TIMESTAMP_TRUNC(lt.block_time, minute)
