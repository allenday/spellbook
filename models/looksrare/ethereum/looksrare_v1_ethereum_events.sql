...
WITH looksrare_trades AS (
    ...
    FROM (
        SELECT
            ...
            ta.evt_index,
            ta.strategy
        FROM {{ source('looksrare_ethereum','LooksRareExchange_evt_TakerAsk') }} AS ta
        {% if is_incremental() %}
        ...
        SELECT
            ...
            tb.evt_index,
            tb.strategy
        FROM {{ source('looksrare_ethereum','LooksRareExchange_evt_TakerBid') }} AS tb
        {% if is_incremental() %}
...
SELECT
    ...
    et.from AS tx_from,
    et.to AS tx_to,
    CAST(COALESCE((pf.fee_percentage / 100) * lr.amount_raw, 0) AS decimal(38, 0)) AS platform_fee_amount_raw,
    COALESCE((pf.fee_percentage / 100) * lr.amount_raw / POWER(10, pu.decimals), 0) AS platform_fee_amount,
    COALESCE((pf.fee_percentage / 100) * pu.price * lr.amount_raw / POWER(10, pu.decimals), 0) AS platform_fee_amount_usd,
    ...
    COALESCE(pu.price * roy.amount / POWER(10, pu.decimals), 0) AS royalty_fee_amount_usd,
...
INNER JOIN {{ source('ethereum','transactions') }} AS et
    ON
        lr.block_time = et.block_time
        AND lr.tx_hash = et.hash
        {% if is_incremental() %}
            AND et.block_time >= date_trunc('day', NOW() - interval '1 week')
        {% endif %}
LEFT JOIN {{ ref('nft_ethereum_aggregators') }} AS agg ON et.to = agg.contract_address
LEFT JOIN {{ ref('tokens_ethereum_nft') }} AS tok ON lr.nft_contract_address = tok.contract_address
LEFT JOIN royalties AS roy
    ON
        roy.block_time = lr.block_time
        AND roy.tx_hash = lr.tx_hash
        AND roy.nft_contract_address = lr.nft_contract_address
        AND roy.token_id = lr.token_id
        AND roy.id = lr.id
LEFT JOIN platform_fees AS pf ON pf.contract_address = lr.strategy
