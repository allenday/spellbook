...
WITH looksrare_trades AS (
    ...
    FROM (
        ...
        {% if is_incremental() %}
        WHERE ta.evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
        {% endif %}
        ...
        {% if is_incremental() %}
        WHERE tb.evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
        {% endif %}
        )
    )
...
, 'v1' AS version
, lr.block_time
, TIMESTAMP_TRUNC(lr.block_time, day) AS block_date
, lr.token_id
...
, CAST(lr.number_of_items AS BIGNUMERIC) AS number_of_items
, lr.trade_category
, 'Trade' AS evt_type
...
, CAST(lr.amount_raw AS BIGNUMERIC) AS amount_raw
...
, et.from AS tx_from
, et.to AS tx_to
, CAST(COALESCE((pf.fee_percentage/100)*lr.amount_raw, 0) as BIGNUMERIC) AS platform_fee_amount_raw
...
, CAST(COALESCE(pf.fee_percentage, 0) AS FLOAT64) AS platform_fee_percentage
...
, CAST(COALESCE(roy.amount, 0) AS BIGNUMERIC) AS royalty_fee_amount_raw
...
, CAST(COALESCE(ROUND(100*roy.amount/lr.amount_raw, 2), 0) AS FLOAT64) AS royalty_fee_percentage
...
LEFT JOIN {{ source('prices','usd') }} pu ON pu.blockchain='ethereum'
    AND pu.minute=TIMESTAMP_TRUNC(lr.block_time, minute)
