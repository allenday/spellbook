...
SELECT DISTINCT
    ...
    TIMESTAMP_TRUNC(pcr.evt_block_time, DAY) AS block_date,
    ...
    CASE 
         WHEN pex.proposalId is not null and CURRENT_TIMESTAMP() > pex.evt_block_time THEN 'Executed' 
         WHEN pca.proposalId is not null and CURRENT_TIMESTAMP() > pca.evt_block_time THEN 'Canceled'
         WHEN pcr.startBlock < pcr.evt_block_number and pcr.evt_block_number < pcr.endBlock THEN 'Active'
         WHEN CURRENT_TIMESTAMP() > pqu.evt_block_time AND startBlock > pcr.evt_block_number THEN 'Queued'
