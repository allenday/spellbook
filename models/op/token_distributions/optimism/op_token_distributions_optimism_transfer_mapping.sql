    ...
    WITH tfers AS (
        -- transfers out
            ...
            COALESCE(
                ...
                cast(tf.value as FLOAT64)/cast( 1e18 as FLOAT64) AS op_amount_decimal,
            ...
            WHERE tf.contract_address IS NULL AND tf.contract_address = '{{op_token_address}}'
            ...
            {% if is_incremental() %} 
            and tf.evt_block_time >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 week')
            {% else %}
        ...
        {% if is_incremental() %} 
            where o.evt_block_time >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 week')
        {% endif %}
        ...
        FROM tfers t
        LEFT JOIN {{ ref('op_token_distributions_optimism_other_distributions_claims') }} o --don't FLOAT64 count - at the amount level b/c there could be multiple claims in one tx
            ...
            {% if is_incremental() %} 
            and o.evt_block_time >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 week')
            {% endif %}
...
SELECT 
    ...
    , op_amount_decimal, tx_method
    --
    , cast(op_claimed as FLOAT64) AS op_claimed
    , cast(op_deployed as FLOAT64) as op_deployed
    , cast(op_to_project as FLOAT64) as op_to_project
    , cast(op_between_projects as FLOAT64) as op_between_projects
    , cast(op_incoming_clawback as FLOAT64) as op_incoming_clawback
    , cast(op_to_utility_contract as FLOAT64) AS op_to_utility_contract
    , cast(op_incoming_other as FLOAT64) AS op_incoming_other
    , cast(op_for_retropgf as FLOAT64) AS op_for_retropgf
    --
