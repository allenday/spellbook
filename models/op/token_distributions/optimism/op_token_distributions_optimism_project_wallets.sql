...
with intermediate_wallets as (
    ...
    from (
        select
            ...
            cast(address_descriptor as varchar(100)) as address_descriptor,
    ...
    where
        rnk = 1 --check to prvent duplicates
...
select
    ...
    concat_ws(',', ARRAY_AGG(distinct fin.address_descriptor)) as address_descriptor,
    concat_ws(
        ',', ARRAY_AGG(
            distinct
...
from (
    select
        ...
        proposal_name,
        address_descriptor,
        ROW_NUMBER() over (partition by address order by rnk asc) as choice_rank
    from (
        ...
        select
            ...
            category as label,
            proposal_name,
            funding_source as address_descriptor,
            1 as rnk
        ...
        select
            ...
            'RetroPGF' as label,
            recipient_name as proposal_name,
            'RetroPGF - ' || round_name as address_descriptor,
            4 as rnk
        from {{ ref('op_retropgf_optimism_recipients') }}

    ) as do_choice_rank
) as fin
left join {{ ref('op_token_distributions_optimism_project_name_mapping') }} as pnm
