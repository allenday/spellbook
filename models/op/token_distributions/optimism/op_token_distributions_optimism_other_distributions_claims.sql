...
aave_lm_claims AS (
    SELECT
        ...
        MAX(evt_tfer_index) AS max_evt_tfer_index,

        (array_agg(
            CASE WHEN claim_rank_asc = 1 THEN from_address END
        ) FILTER (
            WHERE
            CASE WHEN claim_rank_asc = 1 THEN from_address END
            IS NOT NULL
        ...
        (array_agg(
            CASE WHEN claim_rank_asc = 1 THEN to_address END
        ) FILTER (
            WHERE
            CASE WHEN claim_rank_asc = 1 THEN to_address END
            IS NOT NULL
    ...
    FROM (
        SELECT
            ...
            r.evt_tx_hash,
            'Project' AS from_label,
            'Parter Fund' AS from_type,
            'Aave' AS from_name,
            ...
            'Other' AS to_name,
            cast(amount AS double) / cast(1e18 AS double) AS op_amount_decimal,
            --get last
        ...
        FROM {{ source('aave_v3_optimism','RewardsController_evt_RewardsClaimed') }} AS r
        INNER JOIN {{ source('erc20_optimism', 'evt_transfer') }} AS tf
            ON
                tf.evt_tx_hash = r.evt_tx_hash
                ...
                {% endif %}
        LEFT JOIN all_labels AS lbl_from
        ...
        WHERE
            reward = '{{ op_token_address }}' --OP Token
