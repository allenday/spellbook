...
WITH cte_support AS (SELECT
        ...
        , proposalId
        , CASE
            WHEN support = 0 THEN sum(votes / 1e18) ELSE 0
        END AS votes_against
        , CASE WHEN support = 1 THEN sum(votes / 1e18) ELSE 0 END AS votes_for
        , CASE
            WHEN support = 2 THEN sum(votes / 1e18) ELSE 0
        END AS votes_abstain
    FROM
        {{ source('uniswap_v3_ethereum', 'GovernorBravoDelegate_evt_VoteCast') }}
...
FROM
    {{ source('uniswap_v3_ethereum', 'GovernorBravoDelegate_evt_ProposalCreated') }} AS pcr
LEFT JOIN cte_sum_votes ON cte_sum_votes.proposalId = pcr.id
LEFT JOIN
    {{ source('uniswap_v3_ethereum', 'GovernorBravoDelegate_evt_ProposalCanceled') }} AS pca ON
        pca.id = pcr.id
LEFT JOIN
    {{ source('uniswap_v3_ethereum', 'GovernorBravoDelegate_evt_ProposalExecuted') }} AS pex ON
        pex.id = pcr.id
LEFT JOIN
    {{ source('uniswap_v3_ethereum', 'GovernorBravoDelegate_evt_ProposalQueued') }} AS pqu ON
        pex.id = pcr.id
{% if is_incremental() %}
