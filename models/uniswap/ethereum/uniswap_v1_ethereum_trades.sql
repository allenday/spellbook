...
WITH dexs AS (
    ...
    SELECT
        ...
        t.evt_index
    FROM
        {{ source('uniswap_ethereum', 'Exchange_evt_TokenPurchase') }} AS t
    INNER JOIN {{ source('uniswap_ethereum', 'Factory_evt_NewExchange') }} AS f
    ...
    SELECT
        ...
        t.evt_index
    FROM
        {{ source('uniswap_ethereum', 'Exchange_evt_EthPurchase') }} AS t
    INNER JOIN {{ source('uniswap_ethereum', 'Factory_evt_NewExchange') }} AS f
        ON f.exchange = t.contract_address
    {% if is_incremental() %}
        WHERE t.evt_block_time >= date_trunc('day', now() - interval '1 week')
