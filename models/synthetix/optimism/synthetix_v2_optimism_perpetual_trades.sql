...
perps AS (
    SELECT
        ...
        '2' AS version,
        INITCAP(COALESCE(DECODE(UNHEX(SUBSTRING(tr.trackingcode, 3)), 'UTF-8'), 'Unspecified')) AS frontend,
        s.account AS trader,
    ...
    LEFT JOIN asset_price AS p
        ON
            s.contract_address = p.market_address
    ...
    LEFT JOIN {{ source('synthetix_futuresmarket_optimism', 'ProxyPerpsV2_evt_PerpsTracking') }} AS tr
        ON
            s.evt_tx_hash = tr.evt_tx_hash
            AND s.fee = tr.fee
            AND s.tradesize = tr.sizedelta
    WHERE
        CAST(s.tradesize AS DOUBLE) != 0
        {% if is_incremental() %}
