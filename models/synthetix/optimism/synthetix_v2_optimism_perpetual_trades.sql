...
synthetix_markets AS (
	SELECT DISTINCT
		...
		CASE
			WHEN MOD(POSITION('00' IN SUBSTRING(asset, 3)), 2) = 0 THEN TO_BASE64(FROM_HEX(SUBSTRING(asset, 3, POSITION('00' IN SUBSTRING(asset, 3)))))
			ELSE TO_BASE64(FROM_HEX(SUBSTRING(asset, 3, POSITION('00' IN SUBSTRING(asset, 3)))-1))
		END AS asset
		
		,market
		
		,CASE
			WHEN MOD(POSITION('00' IN SUBSTRING(marketKey, 3)), 2) = 0 THEN TO_BASE64(FROM_HEX(SUBSTRING(marketKey, 3, POSITION('00' IN SUBSTRING(marketKey, 3)))))
			ELSE TO_BASE64(FROM_HEX(SUBSTRING(asset, 3, POSITION('00' IN SUBSTRING(marketKey, 3)))-1))
		END AS marketKey
...
perps AS (
	SELECT
		...
		,CASE
		WHEN (CAST(s.margin AS FLOAT64) >= 0 AND CAST(s.size AS FLOAT64) = 0 AND CAST(s.tradeSize AS FLOAT64) < 0 AND s.size != s.tradeSize) THEN 'close'
		WHEN (CAST(s.margin AS FLOAT64) >= 0 AND CAST(s.size AS FLOAT64) = 0 AND CAST(s.tradeSize AS FLOAT64) > 0 AND s.size != s.tradeSize) THEN 'close'
		WHEN CAST(s.tradeSize AS FLOAT64) > 0 THEN 'long'
		WHEN CAST(s.tradeSize AS FLOAT64) < 0 THEN 'short'
		ELSE 'NA'
		END AS trade

		,'Synthetix' AS project
		,'2' AS version
        ,INITCAP(IFNULL(DECODE(TO_BASE64(FROM_HEX(SUBSTRING(tr.trackingCode, 3))), 'UTF-8'), 'Unspecified')) AS frontend
		,s.account AS trader
		,cast(s.tradeSize as FLOAT64) AS volume_raw
	...
	WHERE CAST(s.tradeSize AS FLOAT64) != 0
	{% if is_incremental() %}
...
SELECT
	...
	,SAFE_CAST(TIMESTAMP_TRUNC(perps.block_time, DAY) AS date) AS block_date
	,perps.block_time
	,perps.virtual_asset
