...
(
    ...
    SELECT
        ...
        ,trim('\u0000' from TO_BASE64(FROM_HEX(substring(toCurrencyKey, 3)))) AS token_bought_symbol
        ,trim('\u0000' from TO_BASE64(FROM_HEX(substring(fromCurrencyKey, 3)))) AS token_sold_symbol
    ...
    WHERE t.evt_block_time >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 month')
    {% endif %}
)
SELECT
...
INNER JOIN {{ source('optimism', 'transactions') }} tx
    ...
    {% endif %}
    {% if is_incremental() %}
    AND tx.block_time >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1' month)
    {% endif %}
...
LEFT JOIN {{ source('prices', 'usd') }} p_bought
    ...
    {% endif %}
    {% if is_incremental() %}
    AND p_bought.minute >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 month')
    {% endif %}
LEFT JOIN {{ source('prices', 'usd') }} p_sold
    ...
    {% endif %}
    {% if is_incremental() %}
    AND p_sold.minute >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 month')
    {% endif %}