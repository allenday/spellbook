...
FROM {{ source('ethereum', 'traces') }} AS source_fees
WHERE
...
FROM {{ source('erc20_ethereum', 'evt_transfer') }} AS erc
    LEFT JOIN {{ ref('tokens_erc20') }} AS erc20 ON erc20.contract_address = erc.contract_address AND erc20.blockchain = 'ethereum'
