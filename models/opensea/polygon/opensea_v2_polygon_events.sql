...
WITH trades AS (
   select 
      ...
      case when length(a.rightOrder:makerAssetData) = 650 then cast(udfs.bytea2numeric_v3(substr(a.rightOrder:makerAssetData,332,64)) as string)
            else cast(udfs.bytea2numeric_v3(substr(a.rightOrder:makerAssetData,76,64)) as string)
      end AS token_id,
      ...
      least(cast(output_matchedFillResults:left:makerFeePaid as BIGNUMERIC), cast(output_matchedFillResults:right:takerFeePaid as BIGNUMERIC)) AS amount_raw,
      2.5 AS platform_fee,
...
trade_amount_detail as (
    SELECT e.evt_block_number,
        e.evt_tx_hash,
        CAST(e.value as FLOAT64) AS amount_raw,
...
SELECT
  ...
  'v2' AS version,
  a.tx_hash,
  TIMESTAMP_TRUNC(a.block_time, day) AS block_date,
  a.block_time,
  a.block_number,
  CAST(a.amount_raw / power(10,erc20.decimals) * p.price AS FLOAT64) AS amount_usd,
  CAST(a.amount_raw / power(10,erc20.decimals) AS FLOAT64) AS amount_original,
  CAST(a.amount_raw AS BIGNUMERIC) AS amount_raw,
  ...
  CAST(coalesce(a.number_of_items, 1) AS BIGNUMERIC) AS number_of_items,
  a.trade_category,
  a.buyer,
  a.seller,
  ...
  t.`from` AS tx_from,
  t.`to` AS tx_to,
  CAST(f.platform_fee_amount_raw AS FLOAT64) AS platform_fee_amount_raw,
  CAST(f.platform_fee_amount_raw / power(10,erc20.decimals) AS FLOAT64) AS platform_fee_amount,
  CAST(f.platform_fee_amount_raw / power(10,erc20.decimals) * p.price AS FLOAT64) AS platform_fee_amount_usd,
  CAST(f.platform_fee_amount_raw / a.amount_raw * 100 AS FLOAT64) AS platform_fee_percentage,
  CAST(f.royalty_fee_amount_raw AS FLOAT64) AS royalty_fee_amount_raw,
  CAST(f.royalty_fee_amount_raw / power(10,erc20.decimals) AS FLOAT64) AS royalty_fee_amount,
  CAST(f.royalty_fee_amount_raw / power(10,erc20.decimals) * p.price AS FLOAT64) AS royalty_fee_amount_usd,
  CAST(f.royalty_fee_amount_raw / a.amount_raw * 100 AS FLOAT64) AS royalty_fee_percentage,
...
LEFT JOIN {{ source('prices', 'usd') }} p ON p.minute = TIMESTAMP_TRUNC(a.block_time, minute)
