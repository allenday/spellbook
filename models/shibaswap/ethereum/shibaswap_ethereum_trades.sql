...
{% set project_start_date = '2021-07-06' %}

WITH dexs AS (
    -- Basically a Uniswap v2 CLONE
    SELECT
        ...
        t.evt_index
    FROM
        {{ source('shibaswap_ethereum', 'UniswapV2Pair_evt_Swap') }} AS t
    INNER JOIN {{ source('shibaswap_ethereum', 'UniswapV2Factory_evt_PairCreated') }} AS f
