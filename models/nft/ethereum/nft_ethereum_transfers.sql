...
, 'ethereum' || evt_tx_hash || '-erc1155-' || contract_address || '-' || ids_and_count.ids || '-' || FROM || '-' || to || '-' || ids_and_count.values || '-' || evt_index AS unique_transfer_id
FROM (
    ...
    FROM {{ source('erc1155_ethereum', 'evt_transferbatch') }} t
    {% if is_incremental() %}
        ANTI JOIN {{this}} anti_table
            ON t.evt_tx_hash = anti_table.tx_hash
    {% endif %}
