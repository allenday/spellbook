...
SELECT
    ...
    --, et.to AS tx_to
    app.evt_index
FROM {{ source('erc721_ethereum','evt_Approval') }} AS app
...
SELECT
    ...
    --, et.to AS tx_to
    app.evt_index
FROM {{ source('erc721_ethereum','evt_ApprovalForAll') }} AS app
/*INNER JOIN {{ source('ethereum', 'transactions') }} et ON et.block_number=app.evt_block_number
    AND et.hash=app.evt_tx_hash
{% if is_incremental() %}
AND et.block_time >= date_trunc("day", now() - interval '1 week')
...
SELECT
    ...
    --, et.to AS tx_to
    app.evt_index
FROM {{ source('erc1155_ethereum','evt_ApprovalForAll') }} AS app
/*INNER JOIN {{ source('ethereum', 'transactions') }} et ON et.block_number=app.evt_block_number
    AND et.hash=app.evt_tx_hash
{% if is_incremental() %}
AND et.block_time >= date_trunc("day", now() - interval '1 week')
