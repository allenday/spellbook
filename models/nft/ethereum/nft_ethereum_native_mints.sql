...
, TIMESTAMP_TRUNC(nft_mints.block_time, day) AS block_date
...
, CASE WHEN nft_mints.amount=1 THEN 'Single Item Mint'
    ELSE 'Bundle Mint'
    END AS trade_type
, CAST(nft_mints.amount AS BIGNUMERIC) AS number_of_items
, 'Mint' AS trade_category
, 'Mint' AS evt_type
, nft_mints.from AS seller
, nft_mints.to AS buyer
, CAST(COALESCE(SUM(CAST(et.value as FLOAT64)), SUM(CAST(erc20s.value as FLOAT64)), 0)*(nft_mints.amount/nft_count.nfts_minted_in_tx) AS BIGNUMERIC) AS amount_raw
, COALESCE(SUM(CAST(et.value as FLOAT64))/POWER(10, 18), SUM(CAST(erc20s.value as FLOAT64))/POWER(10, pu_erc20s.decimals))*(nft_mints.amount/nft_count.nfts_minted_in_tx) AS amount_original
, COALESCE(pu_eth.price*SUM(CAST(et.value as FLOAT64))/POWER(10, 18), pu_erc20s.price*SUM(CAST(erc20s.value as FLOAT64))/POWER(10, pu_erc20s.decimals))*(nft_mints.amount/nft_count.nfts_minted_in_tx) AS amount_usd
...
, CAST('0' AS STRING) AS royalty_fee_currency_symbol
, CAST(0 AS FLOAT64) AS royalty_fee_amount_raw
, CAST(0 AS FLOAT64) AS royalty_fee_amount
, CAST(0 AS FLOAT64) AS royalty_fee_amount_usd
...
LEFT JOIN {{ source('prices','usd') }} pu_eth ON pu_eth.blockchain='ethereum'
    AND pu_eth.minute=TIMESTAMP_TRUNC(et.block_time, minute)
    AND pu_eth.contract_address='0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
    {% if is_incremental() %}
    AND  pu_eth.minute >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
LEFT JOIN {{ source('erc20_ethereum','evt_transfer') }} erc20s ON erc20s.evt_block_time=nft_mints.block_time
    ...
    {% if is_incremental() %}
    AND  erc20s.evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
LEFT JOIN {{ source('prices','usd') }} pu_erc20s ON pu_erc20s.blockchain='ethereum'
    AND pu_erc20s.minute=TIMESTAMP_TRUNC(erc20s.evt_block_time, minute)
    AND erc20s.contract_address=pu_erc20s.contract_address
    {% if is_incremental() %}
    AND  pu_erc20s.minute >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
LEFT JOIN {{ source('ethereum','transactions') }} etxs ON etxs.block_time=nft_mints.block_time
    AND etxs.hash=nft_mints.tx_hash
    {% if is_incremental() %}
    AND  etxs.block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
...
{% if is_incremental() %}
AND nft_mints.block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
{% endif %}
