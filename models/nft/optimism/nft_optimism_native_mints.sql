...
select 
    ...
    , cast(coalesce(sum(tr.value), sum(cast(erc20s.value as FLOAT64)), 0)*(nft_mints.amount/nft_count.nfts_minted_in_tx) as BIGNUMERIC) as amount_raw
    , coalesce(sum(tr.value_decimal), sum(cast(erc20s.value as FLOAT64))/power(10, pu_erc20s.decimals))*(nft_mints.amount/nft_count.nfts_minted_in_tx) as amount_original
    , coalesce(pu_eth.price*sum(tr.value_decimal), pu_erc20s.price*sum(cast(erc20s.value as FLOAT64))/power(10, pu_erc20s.decimals))*(nft_mints.amount/nft_count.nfts_minted_in_tx) as amount_usd
...
left join {{ source('prices','usd') }} as pu_eth 
    ...
    and pu_eth.minute=TIMESTAMP_TRUNC(tr.tx_block_time, minute)
