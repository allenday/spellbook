...
LEFT JOIN {{ ref('nft_trades') }} filter_baf
    ...
    AND filter_baf.block_time >= date_trunc("day", CURRENT_TIMESTAMP - interval '2 weeks')
    {% if is_incremental() %}
...
LEFT JOIN {{ ref('nft_trades') }} filter_bought_3x
    ...
    AND filter_bought_3x.block_time >= date_trunc("day", CURRENT_TIMESTAMP - interval '2 weeks')
    {% if is_incremental() %}
