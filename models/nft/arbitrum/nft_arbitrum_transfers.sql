...
, t.to
, t.evt_tx_hash AS tx_hash
, 'arbitrum' || t.evt_tx_hash || '-erc721-' || t.contract_address || '-' || t.id || '-' || t.FROM || '-' || t.to || '-' || t.value || '-' || t.evt_index AS unique_transfer_id
...
, 'arbitrum' || evt_tx_hash || '-erc1155-' || contract_address || '-' || ids_and_count.ids || '-' || FROM || '-' || to || '-' || ids_and_count.values || '-' || evt_index AS unique_transfer_id
FROM (
