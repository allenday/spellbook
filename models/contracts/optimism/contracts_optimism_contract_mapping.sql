...
with base_level as (
    ...
    from (
        ...
        left join {{ ref('contracts_optimism_self_destruct_contracts') }} as sd
            on
                ct.address = sd.contract_address
...
tokens as (
    ...
    inner join {{ ref('tokens_optimism_erc20') }} as t
    ...
    select
        ...
        t.name as symbol
    from base_level as bl
    inner join {{ ref('tokens_optimism_nft') }} as t
        on bl.contract_address = t.contract_address
    group by 1, 2
)
...
{% for i in range(max_levels) -%}
    ,  level{{ i }} as (
        ...
        left join base_level as u --get info about the contract that created this contract
            on
                b.creator_address = u.contract_address
...
{% if is_incremental() %} -- this filter will only be applied on an incremental run 
    left join {{ this }} as th -- grab if the contract was previously picked up as factory created
