...
with nomad_bridge_domainsAS (SELECT * FROM UNNEST(ARRAY<STRUCT<domain_id INT64,domain_name STRING,domain_type STRING>> [STRUCT(6648936, 'Ethereum', 'Outflow'),
STRUCT(1650811245, 'Moonbeam', 'Outflow'),
STRUCT(70901803, 'Moonbeam', 'Inflow'),
STRUCT(1702260083, 'Evmos', 'Outflow'),
STRUCT(73111513, 'Evmos', 'Inflow'),
STRUCT(25393, 'Milkomeda C1', 'Outflow'),
STRUCT(10906210, 'Milkomeda C1', 'Inflow'),
STRUCT(1635148152, 'Avalanche', 'Outflow'),
STRUCT(70229078, 'Avalanche', 'Inflow'),
STRUCT(2019844457, 'Gnosis Chain (xdai)', 'Outflow')]))
...
,nomad_bridge_transactions as (
      select evt_block_time as block_time
          ...
          ,CAST(amount AS FLOAT64) / pow(10, e1.decimals) as original_amount
          ,e1.symbol as original_currency
          ,CAST(amount AS FLOAT64) / pow(10, e1.decimals) * coalesce(p1.price, 0) as usd_amount
          ,`from` as sender
      ...
      left join {{ source('prices', 'usd') }} p1 on p1.contract_address = s.token
            and p1.minute = TIMESTAMP_TRUNC(s.evt_block_time, minute)
      ...
      left join {{ source('prices', 'usd') }} p1 on p1.contract_address = r.token
            and p1.minute = TIMESTAMP_TRUNC(r.evt_block_time, minute)
            and p1.minute >= CAST('2022-01-01' AS TIMESTAMP)
            and p1.blockchain = 'ethereum'
)
