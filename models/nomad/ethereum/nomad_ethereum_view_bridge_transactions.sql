...
nomad_bridge_transactions as (
    ...
    inner join nomad_bridge_domains as d on d.domain_id = CAST(`left`(originandnonce, 8) as BIGINT)
    left join {{ ref('tokens_erc20') }} as e1 on e1.contract_address = r.token and e1.blockchain = 'ethereum'
    left join {{ source('prices', 'usd') }}
        as p1 on p1.contract_address = r.token
