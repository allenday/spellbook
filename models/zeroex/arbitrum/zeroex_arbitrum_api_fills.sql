...
v4_rfq_fills_no_bridge AS (
    ...
    FROM {{ source('zeroex_arbitrum', 'ExchangeProxy_evt_RfqOrderFilled') }} AS fills
...
v4_limit_fills_no_bridge AS (
    ...
    FROM {{ source('zeroex_arbitrum', 'ExchangeProxy_evt_LimitOrderFilled') }} AS fills
    INNER JOIN zeroex_tx
        ON
            zeroex_tx.tx_hash = fills.evt_tx_hash
            AND zeroex_tx.block_number = fills.evt_block_number
...
otc_fills AS (
    SELECT
        ...
        (zeroex_tx.tx_hash IS NOT NULL) AS swap_flag,
        FALSE AS matcha_limit_order_flag
    FROM {{ source('zeroex_arbitrum', 'ExchangeProxy_evt_OtcOrderFilled') }} AS fills
    INNER JOIN zeroex_tx
        ON
            zeroex_tx.tx_hash = fills.evt_tx_hash
            AND zeroex_tx.block_number = fills.evt_block_number
...
direct_plp AS (
    SELECT
        ...
        TRUE AS swap_flag,
        FALSE AS matcha_limit_order_flag
    FROM {{ source('zeroex_arbitrum', 'ExchangeProxy_evt_LiquidityProviderSwap') }} AS plp
    INNER JOIN zeroex_tx
        ON
...
SELECT
    ...
    CASE
        WHEN
            maker_token IN (
                '0x82af49447d8a07e3bd95bd0d56f35241523fbab1', '0xff970a61a04b1ca14834a43f5de4533ebddb5cc8', '0xda10009cbd5d07dd0cecc66161fc93d7c9000da1',
                '0xfc5a1a6eb076a2c7ad06ed22c90d7e710e35ad0a', '0xfd086bc7cd5c481dcc9c85ebe478a1c0b69fcbb9', '0xd74f5255d557944cf7dd0e45ff521520002d5748'
...
LEFT OUTER JOIN {{ ref('tokens_erc20') }} AS ts ON ts.contract_address = taker_token AND ts.blockchain = 'arbitrum'
LEFT OUTER JOIN {{ ref('tokens_erc20') }} AS ms ON ms.contract_address = maker_token AND ms.blockchain = 'arbitrum'
