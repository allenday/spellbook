...
v4_rfq_fills_no_bridge AS (
    ...
    {% if is_incremental() %}
    WHERE fills.evt_block_time >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
    {% if not is_incremental() %}
...
direct_PLP AS (
    ...
    {% if is_incremental() %}
    WHERE plp.evt_block_time >= date_trunc('day', CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
    {% if not is_incremental() %}
...
LEFT JOIN {{ source('prices', 'usd') }} tp 
    ON TIMESTAMP_TRUNC(all_tx.block_time, minute) = tp.minute
    AND CASE
...
LEFT JOIN {{ source('prices', 'usd') }} mp 
    ON TIMESTAMP_TRUNC(all_tx.block_time, minute) = mp.minute
    AND CASE
