...
WITH zeroex_tx AS (
    ...
    FROM {{ source('avalanche_c', 'traces') }} AS tr
    WHERE
        tr.to IN (
            -- exchange contract
...
v4_rfq_fills_no_bridge AS (
    ...
    FROM {{ source('zeroex_avalanche_c', 'ExchangeProxy_evt_RfqOrderFilled') }} AS fills
    INNER JOIN zeroex_tx
        ON
            zeroex_tx.tx_hash = fills.evt_tx_hash
...
v4_limit_fills_no_bridge AS (
    ...
    FROM {{ source('zeroex_avalanche_c', 'ExchangeProxy_evt_LimitOrderFilled') }} AS fills
    INNER JOIN zeroex_tx
        ON
            zeroex_tx.tx_hash = fills.evt_tx_hash
            AND zeroex_tx.block_number = fills.evt_block_number
...
otc_fills AS (
    SELECT
        ...
        (zeroex_tx.tx_hash IS NOT NULL) AS swap_flag,
        FALSE AS matcha_limit_order_flag
    FROM {{ source('zeroex_avalanche_c', 'ExchangeProxy_evt_OtcOrderFilled') }} AS fills
    INNER JOIN zeroex_tx
        ON
            zeroex_tx.tx_hash = fills.evt_tx_hash
            AND zeroex_tx.block_number = fills.evt_block_number
...
newbridgefill AS (
    SELECT
        ...
        TRUE AS swap_flag,
        FALSE AS matcha_limit_order_flag
    FROM {{ source('avalanche_c' ,'logs') }} AS logs
    INNER JOIN zeroex_tx
        ON
            zeroex_tx.tx_hash = logs.tx_hash
...
direct_plp AS (
    SELECT
        ...
        TRUE AS swap_flag,
        FALSE AS matcha_limit_order_flag
    FROM {{ source('zeroex_avalanche_c', 'ExchangeProxy_evt_LiquidityProviderSwap') }} AS plp
    INNER JOIN zeroex_tx
        ON
            zeroex_tx.tx_hash = plp.evt_tx_hash
...
SELECT
    ...
    CASE
        WHEN
            maker_token IN (
                '0x9702230a8ea53601f5cd2dc00fdbc13d4df4a8c7', '0xa7d7079b0fead91f3e65f86e8915cb59c1a4c664', '0xc7198437980c041c805a1edcba50c1ce5db95118',
                '0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7', '0xd586e7f844cea2f87f50152665bcbc2c279d8d70', '0x49d5c2bdffac6ce2bfdb6640f4f80f226bc10bab'
...
LEFT OUTER JOIN {{ ref('tokens_erc20') }} AS ts ON ts.contract_address = taker_token AND ts.blockchain = 'avalanche_c'
LEFT OUTER JOIN {{ ref('tokens_erc20') }} AS ms ON ms.contract_address = maker_token AND ms.blockchain = 'avalanche_c'
