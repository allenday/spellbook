...
erc20bridgetransfer AS (
    SELECT
        ...
        TRUE AS swap_flag,
        FALSE AS matcha_limit_order_flag
    FROM {{ source('fantom', 'logs') }} AS logs
...
bridgefill AS (
    ...
    INNER JOIN zeroex_tx ON zeroex_tx.tx_hash = logs.tx_hash
    WHERE
        topic1 = '0xff3bc5e46464411f331d1b093e1587d2d1aa667f5618f98a95afc4132709d3a9'
    ...
    CASE
        WHEN
            maker_token IN (
                ...
                '0x049d68029688eabf473097a2fc38ef61633a3c7a', '0x321162cd933e2be498cd2267a90534a804051b11'
...
LEFT OUTER JOIN {{ ref('tokens_erc20') }} AS ts ON ts.contract_address = taker_token AND ts.blockchain = 'fantom'
LEFT OUTER JOIN {{ ref('tokens_erc20') }} AS ms ON ms.contract_address = maker_token AND ms.blockchain = 'fantom'
