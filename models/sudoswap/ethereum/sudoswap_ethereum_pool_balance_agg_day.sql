...
WITH
  ...
  erc721_deltas AS (
    SELECT
      TIMESTAMP_TRUNC(et.evt_block_time, day) AS `day`,
      pool_address,
    ...
    {% endif %}
    {% if is_incremental() %}
    WHERE et.evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
    GROUP BY
      1,2
  ),
  ...
  from(
    select * from (
        SELECT
          TIMESTAMP_TRUNC(tr.block_time, day) AS `day`
          , pool_address
