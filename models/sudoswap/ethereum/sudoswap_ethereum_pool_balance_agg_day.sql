...
erc721_deltas AS (
    ...
    FROM
        {{ source('erc721_ethereum','evt_transfer') }} AS et
    INNER JOIN pools AS p
        ON
            p.nft_contract_address = et.contract_address
...
eth_deltas AS (
    ...
    FROM (
        SELECT * FROM (
            ...
            INNER JOIN pools AS pc ON pc.pool_address = tr.to
            WHERE
                tr.success = true
                AND tr.type = 'call'
                AND (
        ...
        SELECT * FROM (
            ...
            FROM
                {{ source('ethereum','traces') }} AS tr
            INNER JOIN pools AS pc ON pc.pool_address = tr.from
            WHERE
                tr.success = true
                AND tr.type = 'call'
                AND (
...
FROM eth_deltas AS e
FULL JOIN erc721_deltas AS n
    ON
        e.day = n.day
