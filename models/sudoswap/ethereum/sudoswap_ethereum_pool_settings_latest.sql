...
latest_pool_fee as (
    ...
    from (
        ...
        from {{ source('sudo_amm_ethereum','LSSVMPair_general_evt_FeeUpdate') }} as evt
        inner join {{ source('ethereum','transactions') }} as tx
            on
                tx.block_time = evt.evt_block_time
...
latest_delta as (
    ...
    from (
        select
            ...
            evt_block_time as update_time,
            row_number() over (partition by contract_address order by evt_block_number desc, tx.index desc) as ordering
        from {{ source('sudo_amm_ethereum','LSSVMPair_general_evt_DeltaUpdate') }} as evt
        inner join {{ source('ethereum','transactions') }} as tx
            on
                tx.block_time = evt.evt_block_time
                and tx.hash = evt.evt_tx_hash
...
latest_spot_price as (
    ...
    from (
        select
            ...
            evt_block_time as update_time,
            row_number() over (partition by contract_address order by evt_block_number desc, tx.index desc) as ordering
        from {{ source('sudo_amm_ethereum','LSSVMPair_general_evt_SpotPriceUpdate') }} as evt
        inner join {{ source('ethereum','transactions') }} as tx
            on
                tx.block_time = evt.evt_block_time
                and tx.hash = evt.evt_tx_hash
...
latest_settings as (
    ...
    from latest_spot_price as t1
    full join latest_delta as t2 on t1.pool_address = t2.pool_address
    full join latest_pool_fee as t3 on t1.pool_address = t3.pool_address or t2.pool_address = t3.pool_address
),
...
{% if is_incremental() %}
    , full_settings_backfilled as (
        select * from (
            ...
            from latest_settings as t1
            full outer join initial_settings as t3
                on t1.pool_address = t3.pool_address
            left join {{ this }} as t2
