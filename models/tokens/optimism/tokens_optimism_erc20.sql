...
SELECT
    *,
    CASE
        WHEN token_type IN ('underlying') THEN 1
...
FROM (
    ...
    FROM {{ ref('tokens_optimism_erc20_transfer_source') }} AS c
    LEFT JOIN {{ ref('tokens_optimism_erc20_curated') }} AS t
        ON c.contract_address = t.contract_address
    LEFT JOIN {{ ref('tokens_optimism_erc20_generated') }} AS b
