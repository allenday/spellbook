...
close_position AS (
    ...
    FROM
        {{ ref('tigris_v1_arbitrum_positions_close') }} AS c
    LEFT JOIN
        open_position AS op
...
liquidate_position AS (
    ...
    FROM
        {{ ref('tigris_v1_arbitrum_positions_liquidation') }} AS lp
    LEFT JOIN
        open_position AS op
            ...
            FROM
                (
                    SELECT
                        ...
                        am.trader
                    FROM
                        {{ ref('tigris_v1_arbitrum_events_add_margin') }} AS am
                    INNER JOIN
                        {{ ref('tigris_v1_arbitrum_positions_leverage') }} AS l
                        ON
...
modify_margin AS (
    ...
    FROM
        {{ ref('tigris_v1_arbitrum_events_modify_margin') }} AS mm
    LEFT JOIN
        open_position AS op
