...
open_position as (
    ...
    FROM {{ ref('tigris_v1_polygon_events_open_position') }}
    {% if is_incremental() %}
...
limit_order as (
    ...
    FROM {{ ref('tigris_v1_polygon_events_limit_order') }}
    {% if is_incremental() %}
    WHERE evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
), 
...
close_position as (
    SELECT 
        TIMESTAMP_TRUNC(c.evt_block_time, day) AS `day`, 
        c.evt_block_time,
        c.evt_index,
        c.evt_tx_hash,
        c.position_id,
    ...
    FROM 
    {{ ref('tigris_v1_polygon_positions_close') }} c 
    LEFT JOIN 
        open_position op 
...
liquidate_position as (
    ...
    FROM 
    {{ ref('tigris_v1_polygon_positions_liquidation') }} lp 
    LEFT JOIN 
        open_position op 
    ...
    {% if is_incremental() %}
    WHERE lp.evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
),
    ...
    (
        ...
        (
            SELECT 
                ...
                am.trader
            FROM 
                {{ ref('tigris_v1_polygon_events_add_margin') }} am 
            INNER JOIN 
                ...
                {% if is_incremental() %}
                AND l.evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
                {% endif %}
            {% if is_incremental() %}
            WHERE am.evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
            {% endif %}
...
modify_margin as (
    ...
    FROM 
        {{ ref('tigris_v1_polygon_events_modify_margin') }} mm 
    LEFT JOIN 
        open_position op 
    ...
    LEFT JOIN 
        ...
        AND mm.version = op.version 
    {% if is_incremental() %}
    WHERE mm.evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
)
