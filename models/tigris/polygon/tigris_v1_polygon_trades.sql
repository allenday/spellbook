...
close_position AS (
    ...
    FROM
        {{ ref('tigris_v1_polygon_positions_close') }} AS c
    LEFT JOIN
        open_position AS op
        ON
            c.position_id = op.position_id
            AND c.version = op.version
    LEFT JOIN
        limit_order AS lo
        ON
            c.position_id = lo.position_id
...
liquidate_position AS (
    ...
    FROM
        {{ ref('tigris_v1_polygon_positions_liquidation') }} AS lp
    LEFT JOIN
        open_position AS op
        ON
            lp.position_id = op.position_id
            AND lp.version = op.version
    LEFT JOIN
        limit_order AS lo
        ON
            lp.position_id = lo.position_id
...
add_margin AS (
            ...
            FROM
                (
                    SELECT
                        ...
                        am.trader
                    FROM
                        {{ ref('tigris_v1_polygon_events_add_margin') }} AS am
                    INNER JOIN
    ...
    LEFT JOIN
        open_position AS op
        ON
            am.position_id = op.position_id
            AND am.version = op.version
    LEFT JOIN
        limit_order AS lo
        ON
            am.position_id = lo.position_id
...
modify_margin AS (
    ...
    FROM
        {{ ref('tigris_v1_polygon_events_modify_margin') }} AS mm
    LEFT JOIN
        open_position AS op
        ON
            mm.position_id = op.position_id
            AND mm.version = op.version
    LEFT JOIN
        limit_order AS lo
        ON
            mm.position_id = lo.position_id
            AND mm.version = op.version
    {% if is_incremental() %}
