...
add_margin_v5 as (
        SELECT 
            TIMESTAMP_TRUNC(ap.evt_block_time, day) AS `day`, 
            ap.evt_tx_hash,
            ap.evt_index,
            ap.evt_block_time,
        ...
        {{ source('tigristrade_polygon', 'TradingV5_call_addToPosition') }} af 
            ...
            {% if is_incremental() %}
            AND af.call_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
            {% endif %}
        {% if is_incremental() %}
        WHERE ap.evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
        {% endif %}
),
