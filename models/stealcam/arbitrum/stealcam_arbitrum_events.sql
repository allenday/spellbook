...
, TIMESTAMP_TRUNC(sc.evt_block_time, day) AS block_date
...
, CAST(sc.value AS BIGNUMERIC) AS amount_raw
, CAST(sc.value/POWER(10, 18) AS FLOAT64) AS amount_original
, CAST(pu.price*sc.value/POWER(10, 18) AS FLOAT64) AS amount_usd
...
, at.from AS tx_from
, at.to AS tx_to
, CAST(0.1*surplus_value AS BIGNUMERIC) AS platform_fee_amount_raw
, CAST(0.1*surplus_value/POWER(10, 18) AS FLOAT64) AS platform_fee_amount
, CAST(pu.price*0.1*surplus_value/POWER(10, 18) AS FLOAT64) AS platform_fee_amount_usd
, CAST(coalesce(100*(0.1*surplus_value/sc.value),0) AS FLOAT64) AS platform_fee_percentage
, 'ETH' as royalty_fee_currency_symbol
, CAST(0.45*surplus_value AS BIGNUMERIC) AS royalty_fee_amount_raw
, CAST(0.45*surplus_value/POWER(10, 18) AS FLOAT64) AS royalty_fee_amount
, CAST(pu.price*0.45*surplus_value/POWER(10, 18) AS FLOAT64) AS royalty_fee_amount_usd
, CAST(coalesce(100*(0.45*surplus_value/sc.value),0) AS FLOAT64) AS royalty_fee_percentage
...
LEFT JOIN {{ ref('prices_usd_forward_fill') }} pu ON pu.blockchain = 'ethereum'
    ...
    AND pu.minute=TIMESTAMP_TRUNC(sc.evt_block_time, minute)
    {% if is_incremental() %}
...
INNER JOIN {{ source('stealcam_arbitrum', 'Stealcam_call_mint') }} m ON m.call_success
    AND m.id=sc.id
    {% if is_incremental() %}
    AND m.call_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
    {% if not is_incremental() %}
