...
**      3. Top MOD( 10, of) retail traders ranked by profit multiple
**
...
swappers as (
    ...
    where 
        blockchain = 'ethereum' 
        and block_date > CURRENT_TIMESTAMP() - interval '60' day
...
, addresses_to_exclude as (
    select distinct address from (
        select distinct cast(address as STRING) as address from {{ ref('labels_contracts') }}
        union all
        select distinct cast(address as STRING) as address from {{ source('ethereum', 'traces') }}
        union all
        select distinct cast(address as STRING) as address from {{ ref('labels_mev_ethereum') }}
        union all
        select distinct cast(address as STRING) as address from {{ ref('labels_sandwich_attackers') }}
    )
)
...
, time_between_trades as (
    select 
        taker,
        ( UNIX_SECONDS(TIMESTAMP(block_time)) - UNIX_SECONDS(TIMESTAMP(lag(block_time, 1)) over (partition by taker order by block_time)) ) / 60  as time_between_trades_mins
    from swappers_address_only
)
...
-- Get latest prices
, prices as (
    select symbol, price, cast(contract_address as STRING) as token_address
...
, buys as (
    select 
        ...
        block_time,
        token_sold_symbol,
        cast(token_sold_address as STRING) as token_sold_address,
        token_bought_symbol,
        cast(token_bought_address as STRING) as token_bought_address,
        token_bought_amount,
        amount_usd, 
        taker,
...
-- Get top MOD( 10, of) retail traders
, percentiles as (
	select taker, 
	    total_profit_USD,
	    profit_multiple,
...
select 
    ...
    taker as address, 
    'Top MOD( 10, Dex) Traders (Retail)' as name, 
    'dex' as category, 
