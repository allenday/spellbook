...
with latest_net_deposits AS ( 
        with annual_flows AS (  
            SELECT 
                d.evt_block_time
                , d.user
                , cast(d.amount as FLOAT64) / pow(10, p.decimals) AS amount
                , d.reserve
                , p.symbol
            ...
            WHERE evt_block_time > CURRENT_TIMESTAMP() - INTERVAL '1' day
            ...
            SELECT 
                w.evt_block_time
                , w.user
                , cast(w.amount as FLOAT64) * -1 / pow(10, p.decimals) AS amount
                , w.reserve
                , p.symbol
...
, latest_available_liquidity AS (
        with cte as (
            SELECT
                ...
                , d.reserve
                , p.symbol
                , cast(d.availableLiquidity as FLOAT64) / pow(10, p.decimals) AS available_liquidity
                , d.call_block_time
