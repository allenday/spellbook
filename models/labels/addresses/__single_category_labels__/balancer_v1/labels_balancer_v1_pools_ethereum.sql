...
state_with_gaps AS (
    SELECT events.block_number, events.pool, events.token, CAST(events.denorm AS FLOAT64) as denorm,
...
CONCAT(STRING_AGG(symbol, '/') , ' ' , STRING_AGG(CAST(norm_weight AS STRING), '/')) AS name,
