...
settings AS (
    SELECT
        ...
        denorm,
        next_block_number
    FROM state_with_gaps AS s
    LEFT JOIN {{ ref('tokens_ethereum_erc20') }} AS t ON s.token = t.contract_address
    WHERE
        next_block_number = 99999999
        AND denorm > 0
),
...
final AS (
    ...
    FROM (
        SELECT
            s1.pool,
            symbol,
            cast(100 * denorm / total_denorm AS integer) AS norm_weight
        FROM settings AS s1
        INNER JOIN (SELECT
            pool,
            sum(denorm) AS total_denorm
        FROM settings GROUP BY pool) AS s2
            ON s1.pool = s2.pool
