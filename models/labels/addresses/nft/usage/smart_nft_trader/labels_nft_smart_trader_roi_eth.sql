    ...
    select 'ethereum'                                                                 AS blockchain,
           ...
           CASE
               WHEN (rank_roi * 1.00 / total_count) * 100 <= 3
                   ...
                   THEN 'Top MOD( 3, Smart) NFT Trader (ROI Realized in ETH w filters)'
               WHEN (rank_roi * 1.00 / total_count) * 100 <= 2
                   AND (rank_roi * 1.00 / total_count) * 100 > 1
                   THEN 'Top MOD( 2, Smart) NFT Trader (ROI Realized in ETH w filters)'
               WHEN (rank_roi * 1.00 / total_count) * 100 <= 1
                   THEN 'Top MOD( 1, Smart) NFT Trader (ROI Realized in ETH w filters)' END AS name,
           'nft'                                                                      AS category,
