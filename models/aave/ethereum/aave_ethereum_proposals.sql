...
WITH cte_support AS (SELECT
        ...
        , id
        , CASE
            WHEN support = 0 THEN sum(votingPower / 1e18) ELSE 0
        END AS votes_against
        , CASE
            WHEN support = 1 THEN sum(votingPower / 1e18) ELSE 0
        END AS votes_for
        , CASE
            WHEN support = 2 THEN sum(votingPower / 1e18) ELSE 0
...
FROM
    {{ source('aave_ethereum', 'AaveGovernanceV2_evt_ProposalCreated') }} AS pcr
LEFT JOIN cte_sum_votes ON cte_sum_votes.id = pcr.id
LEFT JOIN
    {{ source('aave_ethereum', 'AaveGovernanceV2_evt_ProposalCanceled') }} AS pca ON
        pca.id = pcr.id
LEFT JOIN
    {{ source('aave_ethereum', 'AaveGovernanceV2_evt_ProposalExecuted') }} AS pex ON
        pex.id = pcr.id
LEFT JOIN
    {{ source('aave_ethereum', 'AaveGovernanceV2_evt_ProposalQueued') }} AS pqu ON
        pex.id = pcr.id
{% if is_incremental() %}
