...
WITH flashloans AS (
    ...
    , CAST(flash.amount AS FLOAT64) AS amount_raw
    , flash.evt_tx_hash AS tx_hash
    , flash.evt_index
    , CAST(flash.premium AS FLOAT64) AS fee
    ...
    FROM {{ source('aave_v2_polygon','LendingPool_evt_FlashLoan') }} flash
    LEFT JOIN {{ ref('tokens_polygon_erc20') }} erc20 ON flash.asset = erc20.contract_address
    WHERE CAST(flash.amount AS FLOAT64) > 0
    )
...
LEFT JOIN {{ source('prices','usd') }} pu ON pu.blockchain = 'polygon'  
    ...
    AND pu.minute = TIMESTAMP_TRUNC(flash.block_time, minute)