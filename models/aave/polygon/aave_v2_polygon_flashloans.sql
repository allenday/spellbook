...
WITH flashloans AS (
    ...
    FROM {{ source('aave_v2_polygon','LendingPool_evt_FlashLoan') }} AS flash
    LEFT JOIN {{ ref('tokens_polygon_erc20') }} AS erc20 ON flash.asset = erc20.contract_address
...
LEFT JOIN {{ source('prices','usd') }}
    AS pu ON pu.blockchain = 'polygon'
