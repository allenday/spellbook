...
select
    ...
    CASE
        ...
        then cast(atoms_sold as FLOAT64) / cast(limit_sell_amount as FLOAT64)
        else cast(atoms_bought as FLOAT64) / cast(limit_buy_amount as FLOAT64)
        end as fill_proportion,
    CASE when order_type = 'SELL'
        THEN (cast(limit_buy_amount as FLOAT64) / (1.0 - (cast(slippage_bips as FLOAT64) / 10000.0)))
        else cast(limit_buy_amount as FLOAT64) end as buy_quote,
    atoms_bought,
    CASE when order_type = 'BUY'
        THEN (cast(limit_sell_amount as FLOAT64) / (1.0 + (cast(slippage_bips as FLOAT64) / 10000.0)))
        else cast(limit_sell_amount as FLOAT64) end as sell_quote,
    atoms_sold,
