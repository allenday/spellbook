...
sorted_orders as (
    select
        evt_tx_hash,
        evt_block_number,
        ARRAY_AGG(orderUid) as order_ids
    from (
        select
            evt_tx_hash,
            evt_block_number,
...
uid_to_app_id as (
    select
        distinct uid,
        JSON_EXTRACT_SCALAR(trade, '$.appData') as app_data,
        JSON_EXTRACT_SCALAR(trade, '$.receiver') as receiver,
        JSON_EXTRACT_SCALAR(trade, '$.sellAmount') as limit_sell_amount,
        JSON_EXTRACT_SCALAR(trade, '$.buyAmount') as limit_buy_amount,
        JSON_EXTRACT_SCALAR(trade, '$.validTo') as valid_to,
        JSON_EXTRACT_SCALAR(trade, '$.flags') as flags
    from orders_and_trades
        , UNNEST(order_ids) AS uid WITH OFFSET AS i
, UNNEST(trades) AS trade WITH OFFSET AS j
WHERE i = j
),
...
valued_trades as (
    SELECT block_date,
           ...
           valid_to,
           flags,
           case when (MOD( flags, 2)) = 0 then 'SELL' else 'BUY' end as order_type,
