...
with all_superrare_sales as (
    ...
    where
        contract_address = lower('0x8c9f364bf7a56ed058fc63ef81c6cf09c833e656')
    ...
    select
        block_time,
        concat('0x', substring(topic2 from 27 for 40)) as contract_address,
        bytea2numeric_v3(substring(data from 67 for 64)) as token_id,
        concat('0x', substring(topic4 from 27 for 40)) as seller,
        concat('0x', substring(topic3 from 27 for 40)) as buyer,
        bytea2numeric_v3(substring(data from 3 for 64)) as amount,
        tx_hash,
        ''
    from {{ source('ethereum','logs') }}
    where
        contract_address = lower('0x65b49f7aee40347f5a90b714be4ef086f3fe5e2c')
        and topic1 in (
            lower('0x2a9d06eec42acd217a17785dbec90b8b4f01a93ecd8c127edd36bfccf239f8b6'),
...
select distinct
    ...
    ROUND((3 * ((a.amount / 1e18)) / 100), 7) as platform_fee_amount,
    case
    ...
    case
        when
            a.currencyaddress = '0xba5bde662c17e2adff1075610382b9b691296350'
    ...
    coalesce (evt.to, erc20.to) as royalty_fee_receive_address,
    case
        when a.currencyaddress = '0xba5bde662c17e2adff1075610382b9b691296350' then 'RARE'
        else 'ETH' -- only RARE and ETH possible
...
left outer join rare_token_price_eth as rp
    on date_trunc('week', a.evt_block_time) = rp.week
inner join {{ source('ethereum','transactions') }} as t
    on
        a.evt_tx_hash = t.hash
        {% if is_incremental() %}
            and t.block_time >= date_trunc('day', now() - interval '1 week')
        {% endif %}
left outer join {{ source('erc721_ethereum','evt_transfer') }} as evt
    on
        evt.contract_address = a.contract_address
        ...
        {% endif %}
left outer join {{ source('erc20_ethereum','evt_transfer') }} as erc20
    on
        erc20.contract_address = a.contract_address
        ...
        {% endif %}
left outer join transfers_for_tokens_sold_from_auction as po -- if sold from auction house previous owner
