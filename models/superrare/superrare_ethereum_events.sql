...
with all_superrare_sales as (
    ...
    select  block_time
            ...
            , udfs.bytea2numeric_v3(SUBSTR(topic3, 3)) as token_id
            ...
            , udfs.bytea2numeric_v3(SUBSTR(data, 67, 64)) as amount
            , tx_hash
            , ''
    ...
    select block_time
            , concat('0x',SUBSTR(topic1, 27, 40)) as contract_address
            , udfs.bytea2numeric_v3(SUBSTR(data, 67, 64)) as token_id
            , concat('0x',SUBSTR(topic3, 27, 40)) as seller
            , concat('0x',SUBSTR(topic2, 27, 40)) as buyer
            , udfs.bytea2numeric_v3(SUBSTR(data, 3, 64)) as amount
            , tx_hash
            , ''
    from {{ source('ethereum','logs') }}
...
, rare_token_price_eth as (
    select
        TIMESTAMP_TRUNC(block_time, week) AS `week`,
        avg(
            case
...
, transfers_for_tokens_sold_from_auction as (
    ...
    {% if is_incremental() %}
    where evt.evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
)
SELECT distinct
    ...
    'v1' as version,
    cast(TIMESTAMP_TRUNC(a.evt_block_time, day) AS date) AS block_date,
    ...
    CAST(a.amount AS BIGNUMERIC) as amount_raw,
    case
    ...
    CAST('3' AS FLOAT64) as platform_fee_percentage,
    case
    ...
    CAST('10' AS FLOAT64) as royalty_fee_percentage,
    case
...
left outer join
    ...
    on TIMESTAMP_TRUNC(a.evt_block_time, minute) = ep.minute
left outer join rare_token_price_eth rp
    on TIMESTAMP_TRUNC(a.evt_block_time, week) = rp.week
...
left outer join {{ source('erc721_ethereum','evt_transfer') }} evt on evt.contract_address = a.contract_address
    ...
    {% if is_incremental() %}
    and evt.evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
left outer join {{ source('erc20_ethereum','evt_transfer') }} erc20 on erc20.contract_address = a.contract_address
    ...
    {% if is_incremental() %}
    and erc20.evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
    {% endif %}
