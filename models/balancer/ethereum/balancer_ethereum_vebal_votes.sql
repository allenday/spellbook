...
WITH calendar AS (
        SELECT * FROM UNNEST(GENERATE_DATE_ARRAY(DATE('2022-04-07'), CURRENT_DATE(), interval 1 WEEK)) AS start_date
    ),
    
    rounds_info AS (
        SELECT
            start_date,
            UNIX_SECONDS(TIMESTAMP(start_date)) AS start_timestamp,
            date_add(start_date, interval 7 day) AS end_date,
            UNIX_SECONDS(TIMESTAMP(date_add(start_date, interval 7 day))) AS end_timestamp,
   ...
   double_counting AS (
    ...
    votes_with_gaps AS (
        ...
        FROM (
            ...
            AND v.evt_block_time < timestamp(r.end_date)
        ) foo
        ORDER BY 2, 3, 1
    ),
