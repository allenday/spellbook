...
WITH base_locks AS (
        ...
        SELECT provider, CAST(null AS BIGNUMERIC) AS locked_at, locktime AS unlocked_at, ts AS updated_at
    ...
    double_counting AS (
        SELECT
            ...
            block_time,
            updated_at,
            UNIX_SECONDS(TIMESTAMP(block_time)) AS block_timestamp,
            b.provider AS wallet_address,
            bpt_balance,
            unlocked_at,
            bpt_balance / (365*86400) AS slope,
            (unlocked_at - UNIX_SECONDS(TIMESTAMP(block_time))) * bpt_balance / (365*86400) AS bias
        FROM cumulative_balances b
        LEFT JOIN locks_info l
        ON l.provider = b.provider
        AND l.updated_at <= UNIX_SECONDS(TIMESTAMP(block_time))
    ),
