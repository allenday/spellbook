...
WITH lbps_call_create AS (
    ...
    last_weight_update AS (
        ...
        FROM (
            SELECT 
                ...
                timestamp_seconds(CAST(startTime as int64)) AS start_time,
                timestamp_seconds(CAST(endTime as int64)) AS end_time,
    ...
    zipped_lbps_tokens_weights AS (
        ...
        , UNNEST(tokens) as token WITH OFFSET t_pos
        , UNNEST(start_weights) as start_weight WITH OFFSET sw_pos
        WHERE t_pos = sw_pos
    ),
