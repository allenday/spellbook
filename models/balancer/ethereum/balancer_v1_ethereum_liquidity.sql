...
cumulative_balance AS (
    SELECT
        ...
        token,
        cumulative_amount
    FROM {{ ref('balancer_ethereum_balances') }}
    {% if is_incremental() %}
...
cumulative_usd_balance AS (
    ...
    FROM cumulative_balance AS b
    LEFT JOIN {{ ref('tokens_ethereum_erc20') }} AS t ON t.contract_address = b.token
    LEFT JOIN prices AS p1
        ON
            p1.day = b.day
            AND p1.token = b.token
    LEFT JOIN dex_prices
        AS p2 ON p2.day <= b.day
...
pool_liquidity_estimates AS (
    ...
    INNER JOIN {{ ref('balancer_v1_ethereum_pools_tokens_weights') }} AS w
        ON
            b.pool = w.pool_id
...
balancer_liquidity AS (
    ...
    LEFT JOIN {{ ref('balancer_v1_ethereum_pools_tokens_weights') }} AS w
        ON
            b.pool = w.pool_id
            AND w.normalized_weight > 0
    LEFT JOIN {{ ref('tokens_ethereum_erc20') }} AS t ON t.contract_address = w.token_address
