...
fraxswap_dex AS (
    SELECT
        ...
        '' AS trace_address,
        t.evt_index
    FROM {{ source('fraxswap_ethereum', 'FraxswapPair_evt_Swap') }} AS t
    INNER JOIN {{ source('fraxswap_ethereum', 'FraxswapFactory_evt_PairCreated') }} AS p
...
LEFT JOIN {{ ref('tokens_erc20') }} AS erc20a
    ON
        erc20a.contract_address = fraxswap_dex.token_bought_address
        AND erc20a.blockchain = 'ethereum'
LEFT JOIN {{ ref('tokens_erc20') }} AS erc20b
    ON
        erc20b.contract_address = fraxswap_dex.token_sold_address
        AND erc20b.blockchain = 'ethereum'
LEFT JOIN {{ source('prices', 'usd') }} AS p_bought
    ON
        p_bought.minute = date_trunc('minute', fraxswap_dex.block_time)
...
LEFT JOIN {{ source('prices', 'usd') }} AS p_sold
    ON
        p_sold.minute = date_trunc('minute', fraxswap_dex.block_time)
