...
{% set project_start_date = '2022-05-16' %}
...
fraxswap_dex AS (
    SELECT
        ...
        ,''                                                                 AS trace_address
        ,t.evt_index
    FROM {{ source('fraxswap_ethereum', 'FraxswapPair_evt_Swap') }} t
    INNER JOIN {{ source('fraxswap_ethereum', 'FraxswapFactory_evt_PairCreated') }} p
...
SELECT
    ...
    ,'fraxswap'                                                         AS project
    ,'1'                                                                AS version
    ,SAFE_CAST(TIMESTAMP_TRUNC(fraxswap_dex.block_time, DAY) AS date)       AS block_date
    ...
    ,CAST(fraxswap_dex.token_bought_amount_raw AS BIGNUMERIC)        AS token_bought_amount_raw
    ,CAST(fraxswap_dex.token_sold_amount_raw AS BIGNUMERIC)          AS token_sold_amount_raw
...
LEFT JOIN {{ source('prices', 'usd') }} p_bought
    ON p_bought.minute = TIMESTAMP_TRUNC(fraxswap_dex.block_time, minute)
...
LEFT JOIN {{ source('prices', 'usd') }} p_sold
    ON p_sold.minute = TIMESTAMP_TRUNC(fraxswap_dex.block_time, minute)
