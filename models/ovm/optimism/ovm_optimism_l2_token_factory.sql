...
FROM (
    ...
    SELECT
        ...
        c1.call_tx_hash,
        c1.call_block_time,
        c1.call_block_number
    FROM {{ source( 'ovm_optimism', 'L2StandardTokenFactory_call_createStandardL2Token' ) }} AS c1

    INNER JOIN {{ source( 'ovm_optimism', 'L2StandardTokenFactory_evt_StandardL2TokenCreated' ) }} AS tc
        ON
            c1.call_block_number = tc.evt_block_number
    ...
    FROM {{ source( 'ovm_optimism', 'OVM_L2StandardTokenFactory_call_createStandardL2Token' ) }} AS c2

    INNER JOIN {{ source( 'ovm_optimism', 'OVM_L2StandardTokenFactory_evt_StandardL2TokenCreated' ) }} AS tc
        ON
            c2.call_block_number = tc.evt_block_number
