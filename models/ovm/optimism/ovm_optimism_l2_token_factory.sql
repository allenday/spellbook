...
FROM (
    ...
    -- We would need contract function reads to get the actual decimal value - Approximate here, and overwrite in 'tokens_optimism_erc20' as necessary
        ...
        INNER JOIN {{source( 'ovm_optimism', 'L2StandardTokenFactory_evt_StandardL2TokenCreated' ) }} tc
            ...
            {% if is_incremental() %}
            AND tc.evt_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
            {% endif %}
        ...
        WHERE call_success = true
            {% if is_incremental() %}
            AND c1.call_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
            {% endif %}
    ...
    SELECT c2.contract_address, c2._l1Token, _l2Token, _symbol, _name, 
        ...
        WHERE call_success = true
            {% if is_incremental() %}
            AND c2.call_block_time >= date_trunc("day", CURRENT_TIMESTAMP() - interval '1 week')
            {% endif %}
    ) a