...
FROM {{ source('tornado_cash_ethereum','eth_evt_Deposit') }} AS tc
...
SELECT
    tc.evt_block_time AS block_time,
    CASE
        WHEN tc.contract_address = '0xd4b88df4d29f5cedd6857912842cff3b20c8cfa3' THEN '0x6b175474e89094c44da98b954eedeac495271d0f'
    ...
    CASE
        WHEN tc.contract_address = '0xd4b88df4d29f5cedd6857912842cff3b20c8cfa3' THEN 'DAI'
    ...
    tc.contract_address AS contract_address,
    CASE
        WHEN tc.contract_address = '0xd4b88df4d29f5cedd6857912842cff3b20c8cfa3' THEN 100
    ...
    tc.evt_index,
    TRY_CAST(date_trunc('DAY', tc.evt_block_time) AS date) AS block_date
FROM {{ source('tornado_cash_ethereum','erc20_evt_Deposit') }} AS tc
INNER JOIN {{ source('ethereum','transactions') }} AS et
    ON
        et.hash = tc.evt_tx_hash
        {% if not is_incremental() %}
...
SELECT
    ...
    tc.evt_index,
    TRY_CAST(date_trunc('DAY', tc.evt_block_time) AS date) AS block_date
FROM {{ source('tornado_cash_ethereum','ERC20Tornado_evt_Deposit') }} AS tc
INNER JOIN {{ source('ethereum','transactions') }} AS et
    ON
        et.hash = tc.evt_tx_hash
        {% if not is_incremental() %}
