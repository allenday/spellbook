...
SELECT
    ...
    'mdex'                                                        AS project,
    '1'                                                           AS version,
    SAFE_CAST(TIMESTAMP_TRUNC(mdex_dex.block_time, DAY) AS date)      AS block_date,
    mdex_dex.block_time,
    ...
    CAST(mdex_dex.token_bought_amount_raw AS BIGNUMERIC) AS token_bought_amount_raw,
    CAST(mdex_dex.token_sold_amount_raw AS BIGNUMERIC) AS token_sold_amount_raw,
    coalesce(
            mdex_dex.amount_usd
...
LEFT JOIN {{ source('prices', 'usd') }} p_bought
    ON p_bought.minute = TIMESTAMP_TRUNC(mdex_dex.block_time, minute)
...
LEFT JOIN {{ source('prices', 'usd') }} p_sold
    ON p_sold.minute = TIMESTAMP_TRUNC(mdex_dex.block_time, minute)
