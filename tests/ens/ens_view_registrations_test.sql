...
WITH unit_tests AS (
    SELECT COALESCE(test_data.label = ens_vr.label,
        FALSE) AS evt_block_number_test
    FROM {{ ref('ens_view_registrations') }} AS ens_vr
    INNER JOIN {{ ref('ens_view_registrations_postgres') }} AS test_data
...
SELECT
    COUNT(*) AS count_rows,
    COUNT(
        CASE WHEN evt_block_number_test = FALSE THEN 1 END
    ) / COUNT(*) AS pct_mismatch
FROM unit_tests
HAVING
    COUNT(CASE WHEN evt_block_number_test = FALSE THEN 1 END) > COUNT(*) * 0.05
